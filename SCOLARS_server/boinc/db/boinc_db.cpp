...
// D_VAL
// new variables: dist_val, alt_name, validating_wuid, validation_hashed
// BOINC-MR
// new variables: mr_job_id; mr_app_type
void DB_WORKUNIT::db_print(char* buf){
    sprintf(buf,
        ...
        // BOINC-MR
        "mr_job_id=%d, mr_app_type=%d,"
        // D_VAL
        "dist_val=%d, alt_name='%s', validating_wuid=%d, validation_hashed=%d",
        create_time, appid,
        name, xml_doc, batch,
        ...
        priority,
        rsc_bandwidth_bound,
        mr_job_id, mr_app_type,
        dist_val, alt_name, validating_wuid, validation_hashed
    );
}
...
void DB_WORKUNIT::db_parse(MYSQL_ROW &r) {
    ...
    // BOINC-MR
    mr_job_id = atoi(r[i++]);
    mr_app_type = atoi(r[i++]);
    // D_VAL
    dist_val = atoi(r[i++]);
    // obfuscation
    strcpy2(alt_name, r[i++]);
    //alt_name = r[i++];
    validation_hashed = atoi(r[i++]);
}

// D_VAL
// Return list of names of WUs whose files are required and that this host (ID: host_id) executed.
//
// @ dval_state: state of REQ_OUT_FILE_STATE in DB [workunit]
// @ host_id: ID of host to find
/// @ app_name_list: string with list of app names that have requested their results to be returned

/// TODO: only return result if they correspond to the the fastest node (higher n_bwdown)

//
//int DB_WORKUNIT::enumerate_output_files(int dval_state, int host_id, std::string app_name_list, bool use_use_result){
int DB_WORKUNIT::enumerate_output_files(int dval_state, int host_id, bool use_use_result){
    ...
    if (!cursor.active) {
        ...
        /// DEPRECATED - returns all WUs that this host has executed (not just the ones he was the fastest for)
//        sprintf(
//            query,
//            "select DISTINCT "
//            //"workunit.id, workunit.name "
//            "workunit.id, result.name "
//            "FROM workunit, result, host, app "
//            "WHERE "
//            "host.id=result.hostid AND "
//            "result.workunitid = workunit.id AND "
//            "workunit.req_out_file_state=%d AND "
//            "workunit.canonical_resultid>=0 AND "
//            "host.id=%d AND "
//            "result.validate_state=%d AND "             // only look for valid results (don't want to receive the output of an invalid result)
//            "workunit.appid = app.id "
//            "GROUP BY workunit.id ",                    // return only 1 result (first one) [unnecessary?] - we already ignore the following results
//            ///"validation_hashed=0 ",  // unnecessary (since we're requesting outputs, it does not matter if hashed or not - even hashes are returned)
//            //"app.name IN %s ",
//            dval_state, host_id,
...
//        );
        ...
        // return only WUs in which this host has the fastest network connection (largest n_bwdown)
        sprintf(
            query,
            "select result.workunitid, result.name " //, host.id
            "FROM result "
            "inner join ( "
            "   select "
            "   workunitid, max(n_bwdown) as maxnw "
            "   from result, host, workunit "
            "   where host.id = result.hostid and workunit.id = result.workunitid "
            "   and workunit.req_out_file_state=%d and workunit.canonical_resultid>=0 "
            "   and result.validate_state=%d "
            "   group by workunitid "
            ") temp "
            "on temp.workunitid = result.workunitid "
            "inner join host "
            "on host.id = result.hostid and host.n_bwdown = temp.maxnw "
            "WHERE host.id =%d",
            //"app.name IN %s ",
            dval_state,
            VALIDATE_STATE_VALID,
        ...
        // D_VAL DEBUG
        fprintf(stderr,
            "enumerate_output_files: Looking for Workunits that have been executed by fastest host [#%d] with query: %s\n",
            host_id, query
        );
        fflush(stderr);
    ...
    if (!row) {
        ...
        // D_VAL DEBUG
        fprintf(stderr,
            "enumerate_output_files: Did not find a row with the query.\n",
            host_id, query
        );
        fflush(stderr);
        ...
        // D_VAL DEBUG
        fprintf(stderr,
            "enumerate_output_files: Found a row with the query!\n",
            host_id, query
        );
        fflush(stderr);

        // instead of db_parse, only fetch ID and name
        //db_parse(row);
        id = atoi(row[0]);
        strcpy2(name, row[1]);
    }
...
// D_VAL
//
// Return list of names of WUs whose files are required and that this host (ID: host_id) used as input for validation.
// Unlike the enumerate_output_files, which returns the name of WU being validated (since its outputs will be requested), this function
// returns the name of the validator WU (since its input will be requested)
//
// @ dval_state: state of REQ_OUT_FILE_STATE in DB [workunit]
// @ host_id: ID of host to find
// @ client_index: index of result that client should return (it has N clients for each workunit, so return only files with this client index. E.g.: ALTNAME_#CLIENT_#OUT)
/// @ app_name_list: string with list of app names that have requested their results to be returned (and that are not hashed - hashed validators do not have the necessary
//                                                                                              input, so do not include them in query)
//
//int DB_WORKUNIT::enumerate_input_files(int dval_state, int host_id, int& validated_wuid, std::string app_name_list, bool use_use_result){
int DB_WORKUNIT::enumerate_input_files(int dval_state, int host_id, int& validated_wuid, int& client_index, bool use_use_result){
    char query[MAX_QUERY_LEN];
    MYSQL_ROW row;
    int retval;
...
//        // Group By clause necessary in case this host executed more than one WU responsible for the validation the same WU (id: validated_wuid)
//        sprintf(
//            query,
//            "select DISTINCT "
//            // ask for validated WU's alternative name instead of validator WU's name (has a result ID appended. e.g. "ax4cafs2a_21" instead of simply "ax4cafs2a")
//            // validator WU input file name template: 'validated WU alt name'_#client_#outputfile (e.g.: ax4cafs2a_0_0, ax4cafs2a_1_0, etc.)
//            "wu2.alt_name, wu2.id "
//            "FROM workunit wu1, workunit wu2, result, host, app "
//            "WHERE "
//            "host.id=result.hostid AND "
//            "result.workunitid = wu1.id AND "
//            "wu2.id=wu1.validating_wuid AND "
//            "wu2.req_out_file_state=%d AND "
//            "host.id=%d AND "
//            "wu2.appid = app.id AND "
//            "wu1.validation_hashed = 0 "        // did not use hashes to validate, so the host holds the necessary input files
//            //"app.name IN %s "
//            "group by wu2.id ",
//            dval_state, host_id
//            //app_name_list.c_str()
...
//
        ...
        sprintf(
            ...
            // ask for validated WU's alternative name instead of validator WU's name (has a result ID appended. e.g. "ax4cafs2a_21" instead of simply "ax4cafs2a")
            // validator WU input file name template: 'validated WU alt name'_#client_#outputfile (e.g.: ax4cafs2a_0_0, ax4cafs2a_1_0, etc.)
            "wu2.alt_name, wu2.id, res2.validate_state "
            "FROM workunit wu1, workunit wu2, result res1, result res2, host, app "
            "WHERE "
            "host.id=res1.hostid AND "
            "res1.workunitid = wu1.id AND "
            "res2.workunitid = wu2.id AND "
            "wu2.id=wu1.validating_wuid AND "
            "wu2.req_out_file_state=%d AND "
            "host.id=%d AND "
            "wu2.appid = app.id AND "
            "wu2.canonical_resultid > 0 AND "   // only look for workunits that have been validated [set canonical_resultid when validator is successfully finished validating D_VAL WU]
            "res2.validator_wuid<=wu1.id AND "   // look for results that have been validated by this host/wu [wu1] (hostid = res1.id; res1.workunitid = wu1.id)
            "wu1.validation_hashed = 0 "        // did not use hashes to validate, so the host holds the necessary input files
            "order by res2.id ",
            dval_state, host_id
        );
        ...
        // D_VAL DEBUG
        fprintf(stderr,
            "enumerate_input_files: Looking for Workunits that have been validated by host [#%d] with query: %s\n",
            host_id, query
        );
        fflush(stderr);
    ...
    do {
        ...
        if (!row) {
            ...
            // D_VAL DEBUG
            fprintf(stderr, "No more rows found. validate_states size: %zu\n", validate_states.size());
            fflush(stderr);
            ...
            if (validate_states.size() == 0){
                // D_VAL DEBUG
                fprintf(stderr, "enumerate_input_files: Did not find a row with the query. Returning: %d\n", ERR_DB_NOT_FOUND);
                fflush(stderr);
            ...
            // D_VAL DEBUG
            fprintf(stderr, "enumerate_input_files: Found a row with the query! Wu alt name: %s | validated_wuid: %s | validate_state: %s\n",
                    row[0], row[1], row[2]);
            fflush(stderr);

            // check if this is the first workunit found or if we are still dealing with the same workunit
            // If so, save the information, and compile information on different results' validate_state
            if (!name || strcmp(name, "") == 0 || strcmp(name, row[0]) == 0){

                // D_VAL DEBUG
                fprintf(stderr, "Saving information found! alt name: %s | validated_wuid: %s | validate_state: %s\n",
                        row[0], row[1], row[2]);
                fflush(stderr);

                // instead of db_parse, only fetch ID and name of validator WU and ID of validated WU
                //db_parse(row);
                ///id = atoi(row[0]);       // deprecated - unnecessary
                strcpy2(name, row[0]);
                validated_wuid = atoi(row[1]);
                validate_states.push_back(atoi(row[2]));

                // check if this workunit had a valid result
                if (atoi(row[2]) == VALIDATE_STATE_VALID)
                    found_valid_result = true;
            }
            // if the previous workunit found had no valid results, move on to the next one
            else if(!found_valid_result){

                // D_VAL DEBUG
                fprintf(stderr, "enumerate_input_files. Found another workunit, but last had no valid results. Previous wu(alt_name): %s | current wu.alt_name: %s\n",
                        name, row[0]);
                fflush(stderr);

                validate_states.clear();
                strcpy2(name, row[0]);
                validated_wuid = atoi(row[1]);
            ...
            // if we're already reached a different workunit in this row (and previous wu had a valid result), leave it for the next pass
            else{
                // D_VAL DEBUG
                fprintf(stderr, "enumerate_input_files. Found another workunit different from the last. Previous wu(alt_name): %s | current wu.alt_name: %s\n",
                        name, row[0]);
                fflush(stderr);
                break;
            }
    ...
    std::vector<int> valid_client_indexes;

    // check if no result was found
    if (validate_states.size() == 0){
        // D_VAL DEBUG
        fprintf(stderr, "enumerate_input_files. Error. Did not find a row with the query.\n");
        fflush(stderr);
        return ERR_DB_NOT_FOUND;
    }

    for (int i=0; i<validate_states.size(); i++){
        if (validate_states[i] == VALIDATE_STATE_VALID)
            valid_client_indexes.push_back(i);
    }

    // D_VAL DEBUG
    fprintf(stderr,"enumerate_input_files: validate_states (size): %zu): ", validate_states.size());
    fflush(stderr);

    // D_VAL DEBUG
    fprintf(stderr,"enumerate_input_files: List of valid client indexes (size: %zu): ", valid_client_indexes.size());
    fflush(stderr);
    for (int j=0; j<valid_client_indexes.size(); j++){
        fprintf(stderr, "%d | ", valid_client_indexes[j]);
        fflush(stderr);
    }
    // D_VAL DEBUG
    fprintf(stderr,"\n");
    ...
    if (valid_client_indexes.size() <= 0){
        // D_VAL DEBUG
        fprintf(stderr,
            "enumerate_input_files: Did not find a valid client index (host who had returned a valid result for the validated WU [%d]).\n",
            validated_wuid
        );
        fflush(stderr);
        return ERR_DB_NOT_FOUND;
    ...
    int chosen_index = (int)rand()%(int)valid_client_indexes.size();

    client_index = valid_client_indexes[chosen_index];

    // D_VAL DEBUG
    fprintf(stderr,
        "enumerate_input_files: Chose client index: [#%d]\n",
        client_index
    );
    fflush(stderr);
...
//        // D_VAL DEBUG
//        fprintf(stderr,
//            "enumerate_input_files: Did not find a row with the query.\n",
//            host_id, query
//        );
//        fflush(stderr);
//
...
//        // D_VAL DEBUG
//        fprintf(stderr,
//            "enumerate_input_files: Found a row with the query!\n",
//            host_id, query
//        );
//        fflush(stderr);
//
...
//        strcpy2(name, row[0]);
//        validated_wuid = atoi(row[1]);
//        ///appid = atoi(row[3]); // deprecated (now we get a list of valid wu.appid to query DB for)
//    }
    return 0;
...
// BOINC-MR
// Added new column - user_hosting (smallint(6) - boolean)
/// D_VAL TODO: check if this stays in RESULT or moves to WORKUNIT
// added new column - validator_wuid (int)
// validating_wuid (int) [DEPRECATED]
...
//
// BOINC-MR
// Added (next-to-last) value to Result table (column user_hosting [smallint(6)] : 0)
/// D_VAL TODO - goes here or WORKUNIT
// added last two values to Result (columns validating_wuid [int] DEPRECATED; validator_wuid [int])
void DB_RESULT::db_print_values(char* buf){
    ...
    sprintf(
        ...
        "%d, %d, %d, "
        "%.15e, %.15e, %f, %d, "
        //"%d, %d, %d, %d, %d, null, 0, 0, 0, 0, %d, %d)",
        "%d, %d, %d, %d, %d, null, 0, 0, 0, 0, %d)", // CHANGE TO THIS LINE (delete validating_wuid)
        create_time, workunitid,
        server_state, outcome, client_state,
        hostid, userid,
        report_deadline, sent_time, received_time,
...
// BOINC-MR
// Parse two MR values from WU table - mr_app_type, mr_job_id
// D_VAL
// Parse two values from WU table - dist_val; alt_name; one from RESULT table - validator_wuid
// Must match enumerate below
void TRANSITIONER_ITEM::parse(MYSQL_ROW& r) {
    ...
    // D_VAL
    dist_val = safe_atoi(r[i++]);
    strcpy2(alt_name, r[i++]);
    validator_wuid = safe_atoi(r[i++]);
    validating_wuid = safe_atoi(r[i++]);
}

// BOINC-MR
// Get two extra items from DB [WU table]: mr_app_type and mr_job_id
//D_VAL
// Get four extra items from DB [WU]: dist_val, alt_name, validating_wuid; [RES] validator_wuid
...
// D_VAL
// added validator_wuid - set by transitioner after creating validator WUs (updating D_VAL results)
int DB_TRANSITIONER_ITEM_SET::update_result(TRANSITIONER_ITEM& ti) {
    char query[MAX_QUERY_LEN];

    sprintf(query,
        "update result set server_state=%d, outcome=%d, "
        "validate_state=%d, file_delete_state=%d, validator_wuid=%d where id=%d",
        ti.res_server_state,
        ti.res_outcome,
...
void VALIDATOR_ITEM::parse(MYSQL_ROW& r) {
    ...
    // D_VAL
    wu.validating_wuid = atoi(r[i++]);
}

// D_VAL
// added validating_wuid
...
void WORK_ITEM::parse(MYSQL_ROW& r) {
    ...
    // BOINC-MR
    wu.mr_job_id = atoi(r[i++]);
    wu.mr_app_type = atoi(r[i++]);
}
...
//
// BOINC-MR
// parse xml_doc_in: required to check if this result is part of MapReduce job
...
// BOINC-MR
// Added user_hosting to changed attribute of Result reported by user
...
const char *BOINC_RCSID_ac374386c8 = "$Id: boinc_db.cpp 19508 2009-11-06 23:31:26Z davea $";
