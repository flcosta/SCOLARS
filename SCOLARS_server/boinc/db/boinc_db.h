...
/// BOINC-MR TODO : check for errors / problems
/// hack: increase xml_doc size to allow the creation of a reduce workunit after receiving hashes...
#define MEDIUM_BLOB_SIZE                        162144
...
struct WORKUNIT {
    ...
    char name[256];
    /// BOINC-MR TODO : read above - hack to allow bigger wu_templates when creating reduce wu from hashes (with md5_cksum and nbytes info)
    ...
    // BOINC-MR
    //bool is_mapreduce; // unnecessary: can check with (mr_job_id <0?)
    int mr_job_id;
        // set by project admin when adding a new job to mr_jobtracker.xml - read when create_work() [create_work application] is called
    int mr_app_type;
        // either Map or Reduce [default values on mr_jobtracker.h] MR_APP_TYPE_REDUCE; MR_APP_TYPE_MAP

    int dist_val;
        // distributed validation: set to 1 if this workunit is going to be validated by another client, instead of the server

    int obfusc;
        // obfuscation used in this workunit: set to 1 if this workunit has an alternative name, used for obfuscation purposes (e.g., between workflow stages)
        // obfuscation is used implicitily if dist_val = 1 (workunit has an alternative name)

    //std::string alt_name; - deprecated
    char alt_name[256];
        // workunit's alternative name, used for obfuscation purposes. E.g., client validator requests inputs from other clients with different names from the originals.
        // Validator requests file abc_0; original name: ngram_wu_0

    int validating_wuid;
        // ID of workunit that this result is validating. Used in distributed validation
        // (client runs a workunit that validates another WU with id: validating_wuid)

    bool validation_hashed;
        // true if the validation of this workunit is performed through hashes (instead of actual output files)
    ...
    // Constructor to set workunits as default to non-MapReduce tasks without distributed validation
};
...
// BOINC-MR
#define RESULT_SERVER_STATE_MR_INC_INPUTS  7
    // Reduce work unit and results have been created,
    // but not all map outputs are available as input (not finished or validated)
...
// BOINC-MR
#define RESULT_OUTCOME_MR_MAP_UNAVAILABLE 8
    // Not all the map outputs are available as input for this Reduce task
...
#define VALIDATE_STATE_UNDER_DIST_VAL 100
    // this result is currently undergoing distributed validation, therefore it should
    // not be considered for local validation (or viewed as a new result available for server validation)

// BOINC-MR
// values to identify type of result as Map or Reduce task
#define MR_MAP_RESULT       0
#define MR_REDUCE_RESULT    1
...
struct RESULT {
    ...
    /// BOINC-MR TODO - if changed here, must add this column to DB (no, only if DB_RESULT is changed)
    int mr_deadline;            // deadline for all (Map) inputs to be available
    bool is_mapreduce;          // whether this result is part of MapReduce job
    int mr_job_id;              // MapReduce job ID
    int mr_job_type;            // Map or Reduce (defined on top)
    RESULT(){
        is_mapreduce = false;
        mr_job_id = 0;
        mr_deadline = 0;
    ...
    // Needed for DB_RESULT - saved in BOINC DB
    bool user_hosting;
        // True [1] if user/host that returned this result is hosting this output.
        // Use int instead of bool because it is saved as BIT in db (easier to convert)

    /// DEPRECATED? Only on WU
    /// D_VAL TODO : check if this is set here or in the WORKUNIT
    //int validating_wuid;
        // ID of workunit that this result is validating. Used in distributed validation
        // (client runs a workunit that validates another WU with id: validating_wuid)
    int validator_wuid;
        // ID of 1st workunit responsible for validating this result (1st workunit -> after this result is received by server, the validator wu is created
        // if there are n > min_quorum results; if further results are returned, more WUs are created. Only the ID of 1st WU that validates this result is saved)
};
...
// VMR
// D_VAL : dist_val [WU], alt_name [WU], validator_wuid [RES], validating_wuid [WU]
struct TRANSITIONER_ITEM {
    ...
    int mr_job_id;
    int mr_app_type;
        // both required for transitioner to check which WUs and Results need to have their MAP outputs moved to input directory

    // Distributed validation
    int dist_val;
        // required for transitioner to know if this WU and results are supposed to be validated by another client
    char alt_name[256];
        // alternative name given to WORKUNIT
    int validator_wuid;
        // ID of WU that is validating this Result
    int validating_wuid;
        // ID of WU that is being validated by this WU
...
public:
    ...
    // D_VAL
//    int enumerate_output_files(int, int, std::string, bool use_use_result=false);
//    int enumerate_input_files(int, int, int&, std::string, bool use_use_result=false);
    int enumerate_output_files(int, int, bool use_use_result=false);
    int enumerate_input_files(int, int, int&, int&, bool use_use_result=false);
};
...
struct SCHED_RESULT_ITEM {
    ...
    // BOINC-MR
    int user_hosting;
        // True (1) if user is hosting this result's Map outputs, False (0) otherwise
    char xml_doc_in[BLOB_SIZE];
