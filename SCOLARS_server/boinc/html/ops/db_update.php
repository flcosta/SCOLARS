...
function update_1_5_2011(){
    do_query("alter table result add (
        user_hosting smallint not null)");
    do_query("alter table workunit add (
		mr_job_id int default -1,
		mr_app_type smallint default -1)");
}

function update_9_1_2013(){
    // ID of previous workunit being validated by this WU
    //do_query("alter table result add (
    //       validating_wuid int default -1)");
    do_query("alter table workunit add (
		dist_val smallint default 0)");
//    do_query("CREATE TABLE	dist_validation
//	( resultid INT(11),
//	  workunitid INT(11),
//	  PRIMARY KEY (resultid, workunitid) )");
}

// update to allow obfuscation between tasks (e.g., dist validation or workflows)
function update_2_1_2014(){
    do_query("alter table workunit add (
                alt_name varchar(256))");
}


// update to allow validating workunits to know which previous wu they are handling
function update_1_4_2014(){
// ID of previous workunit being validated by this WU
    do_query("alter table workunit add (
           validating_wuid int default -1)");
}

// save information on each d_val result on which workunit is responsible for validating it (validator_wuid)
function update_2_4_2014(){
// ID of previous workunit being validated by this WU
    do_query("alter table result add (
           validator_wuid int default -1)");
}

// current state of request for output file from d_val WUs 
function update_1_5_2014(){
    do_query("alter table workunit add (
           validation_hashed smallint default 0)");
    do_query("alter table workunit add(
	   req_out_file_state smallint default -1)");
}
