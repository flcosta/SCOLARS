...
void replace_file_name(char* xml_doc, char* filename, char* new_filename) {
    /// BOINC-MR TODO - cleanup hack increased wu.xml_doc size
...
void make_new_wu(DB_WORKUNIT& original_wu, char* starting_xml, int start_time) {
    ...
    /// BOINC-MR TODO : cleanup - hack to allow wu.xml_doc to increase size to create Red template
    /// starting_xml = wu.xml_doc
    //char new_buf[BLOB_SIZE];
    char new_buf[MEDIUM_BLOB_SIZE];
    char * p;
    int retval;
...
const char *BOINC_RCSID_d24265dc7f = "$Id: make_work.cpp 19079 2009-09-17 17:56:59Z davea $";
