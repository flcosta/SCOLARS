...
// BOINC-MR
// check if:
// - this is a map result from a MapReduce job, in which only the hashes should have been returned (look for <mr_hash/> tag)
// - the client was not able to host these files, which means that the actual outputs were returned (!result.user_hosting)
// If these conditions hold, verify that the output files match the hashes returned.
int get_output_file_infos(RESULT& result, vector<FILE_INFO>& fis) {
    ...
    while (!xp.get(tag, sizeof(tag), is_tag)) {
        ...
        if (!strcmp(tag, "mr_hash")) {
            mr_res = true;
        }
    }

    // if this is a result from a MR job with hashes, and the user is not hosting the map output file(s)
    if (mr_res && !result.user_hosting){
        char md5_comp[MD5_LEN], md5_reported[MD5_LEN];
        FILE_INFO fi, fi_hash;
        double nbytes;
        int half_total = fis.size()/2;
        char hash_buf[256];
        FILE* f;
        for (int j=0; j< half_total; j++){
            // compute each file's hash
            fi = fis[j];
            fi_hash = fis[j+half_total];
            int retval = md5_file(fi.path.c_str(), md5_comp, nbytes);
            if (retval) {
                log_messages.printf(MSG_CRITICAL,
                    "[RESULT#%d %s] get_output_file_infos() - Couldn't open %s\n",
                    result.id, result.name, fi.path.c_str()
                );
                return retval;
            }

            // open each of the files and read the third line (1st line with metadata, second line with file size)
            f = fopen(fi_hash.path.c_str(), "r");
            if (!f){
                perror("Unable to open file");
                log_messages.printf(MSG_CRITICAL,
                        "get_output_file_infos() - Errno: %d. Unable to open file: %s\n",
                        errno, fi.path.c_str()
                );
                return ERR_FOPEN;
            }
            ...
            if (fgets(hash_buf, 256, f) == NULL) {
                log_messages.printf(MSG_CRITICAL,
                        "get_output_file_infos() - Unable to read file [Map hashed output]: %s\n",
                        fi.path.c_str()
                );
                fclose(f);
                return ERR_FREAD;
            ...
            if (fgets(hash_buf, 256, f) != NULL) {
                // compare computed hash with hash returned
                if (strcmp(hash_buf, md5_comp) == 0){
                    // BOINC-MR DEBUG
                    log_messages.printf(MSG_DEBUG,
                            "get_output_file_infos() - Read hash [%s] for file: %s\n",
                            hash_buf, fi.path.c_str()
                    );
                    continue;
                }
                else {
                    // Error: hash returned does not match hash computed from file
                    log_messages.printf(MSG_CRITICAL,
                            "get_output_file_infos() - Error: Hash returned by Map hashed output file does not match the hash computed from the file. Returned: %s | File hash: 5%s\n",
                            fi.path.c_str(), hash_buf, md5_comp
                    );
                }
            }
            else{
                log_messages.printf(MSG_CRITICAL,
                        "get_output_file_infos() - Unable to read hash in file [Map hashed output]: %s\n",
                        fi.path.c_str()
                );
                fclose(f);
                return ERR_FREAD;
...
const char *BOINC_RCSID_07049e8a0e = "$Id: validate_util.cpp 18831 2009-08-12 16:26:43Z davea $";
