...
// D_VAL
#include "distributed_validation.h"         // required to check if current work request was sent by client that is holding required output/input files from D_VAL WU
...
// D_VAL
// look for WORKUNIT in DB whose files must be requested from clients hosting them <=> [DB] WORKUNIT.REQ_OUT_FILE_STATE == D_VAL_RESQUEST_FILE
// Check if host responsible for current request executed a RESULT of any of those Workunits with required files.
// For every workunit found, update its REQ_OUT_FILE_STATE to D_VAL_REQUEST_SENT
//
// @ host_id: ID of host that made the request (match against WUs)
//
int d_val_check_host_requested_files(int host_id){
    DB_WORKUNIT wu;
    char buf[256], buf2[256];
    int retval = -1;
    //std::stringstream dval_app_names;
    int i;

    srand(time(0));

    // check for output files (D_VAL WUs executed by this host)
    // enumerate not enough (SQL query only reaches workunit table)
    //while (!(retval = wu.enumerate_output_files(D_VAL_REQUEST_FILE, host_id, dval_app_names.str()))) {
    while (!(retval = wu.enumerate_output_files(D_VAL_REQUEST_FILE, host_id))) {
        // what is returned inside wu.name is the result.name for a valid wu's result (use it to ask for files, instead of returning all results from that workunit)
        g_reply->dval_result_output_files.push_back(wu.name);
        // D_VAL DEBUG
        log_messages.printf(MSG_DEBUG,
                "Found a WU executed by this host [#%d]: WU [#%d] - RESULT [%s]. Updating its req_out_file_state to D_VAL_REQUEST_SENT\n",
                host_id, wu.id, wu.name);

        // update workunit's req_out_file_state
        sprintf(buf2, "req_out_file_state=%d", D_VAL_REQUEST_SENT);
        wu.update_field(buf2);
        //wus_found.push_back(wu.id);
    }

    // D_VAL DEBUG
    if (retval){
        log_messages.printf(MSG_DEBUG,
                "[ERR_DB_NOT_FOUND] Did not find a WU executed by this host [#%d]. Retval: %d | ERR_DB_NOT_FOUND: %d\n", host_id, retval, ERR_DB_NOT_FOUND);
    }


    // check for input files - D_VAL WUs that were validated by this host
    // parse all existing D_VAL_APP but do not consider those with <hashed/>
//    dval_app_names.str("");
//    dval_app_names << "(";
//
//    first_element = true;
...
//        if (dval_temp->must_return_result() && !dval_temp->is_hashed()){
//            if (first_element)
//                first_element = false;
//            else
//                dval_app_names << ",";
//
//            dval_app_names << "\"" << dval_temp->get_app_name_wu_to_validate() << "\"";
//        }
//    }
//    dval_app_names << ")";
    ...
    // D_VAL DEBUG
//    log_messages.printf(MSG_DEBUG,
//                "Looking for WU inputs with apps IN: %s\n", dval_app_names.str().c_str());

    //int i;
    int validated_wuid, client_index = -1;
    std::stringstream ss_request_input;
    //while (!wu.enumerate_input_files(D_VAL_REQUEST_FILE, host_id, validated_wuid, dval_app_names.str())) {
    while (!(retval = wu.enumerate_input_files(D_VAL_REQUEST_FILE, host_id, validated_wuid, client_index))) {
//    retval = wu.enumerate_input_files(D_VAL_REQUEST_FILE, host_id, validated_wuid, client_index);

        ss_request_input.str("");

        if (client_index < 0){
            log_messages.printf(MSG_INTERNAL_ERROR,
                    "d_val_check_host_requested_files. Error when trying to identify a client that had returned a valid result for Workunit [#%d]. Client index: %d\n",
                    validated_wuid, client_index);
            break;
        }

        // D_VAL DEBUG
//        log_messages.printf(MSG_DEBUG,
//                "Found a WU validated by this host [#%d]. Wu alt name: %s | client_index: %d\n",
//                host_id, wu.name, client_index);

        ss_request_input << wu.name << "_" << client_index;

        //g_reply->dval_wu_input_files.push_back(wu.name);
        g_reply->dval_wu_input_files.push_back(ss_request_input.str());

        // D_VAL DEBUG
//        log_messages.printf(MSG_DEBUG,
//                "Parsed both values together. ss_request_input: %s\n",
//                ss_request_input.str().c_str());

        // D_VAL DEBUG
        log_messages.printf(MSG_DEBUG,
                "Found a WU validated by this host [#%d]. Requesting input files from workunit [%s] for client [#%d]. Updating its req_out_file_state to D_VAL_REQUEST_SENT. request_input:%s\n",
                host_id, wu.name, client_index, ss_request_input.str().c_str());

        //DB_WORKUNIT wu;
        //wu.id = result.workunitid;
        sprintf(buf2, "req_out_file_state=%d", D_VAL_REQUEST_SENT);
        // update this value on the WU that was validated by wu
        wu.id = validated_wuid;
        wu.update_field(buf2);
        client_index = -1;

//        retval = wu.enumerate_input_files(D_VAL_REQUEST_FILE, host_id, validated_wuid, client_index);
//
//        fprintf(stderr, "Inside loop, After calling wu.enumerate_input_files again. Loop back if retval !=0. Retval: %d\n", retval);
//        fflush(stderr);

//        if(retval != 0){
//            fprintf(stderr, "Retval !=0 is true\n", retval);
//            fflush(stderr);
//        }
//        if (!retval){
//            fprintf(stderr, "!retval is true\n", retval);
//            fflush(stderr);
//        }

        //D_VAL DEBUG
//        log_messages.printf(MSG_DEBUG,
//                    "Inside loop, After calling wu.enumerate_input_files again. Loop back if retval !=0. Retval: %d\n", retval);

    }

//    fprintf(stderr, "Just left loop! before log_messages.printf\n");
//    fflush(stderr);

//    log_messages.printf(MSG_DEBUG, "Test of log_messages.");
//    fflush(stderr);
...
//    // D_VAL DEBUG
//    log_messages.printf(MSG_DEBUG,
//                "wu.enumerate_input_files. Retval: %d\n", retval);
//    if (retval){
//        log_messages.printf(MSG_DEBUG,
//                "[ERR_DB_NOT_FOUND] Did not find a WU executed by this host [#%d]. Retval: %d | ERR_DB_NOT_FOUND: %d\n", host_id, retval, ERR_DB_NOT_FOUND);
//    }
    ...
    // D_VAL DEBUG
    log_messages.printf(MSG_DEBUG,
                "Found %zu WUs that were executed by this host [#%d]\n", g_reply->dval_result_output_files.size(), host_id);
    fflush(stderr);
    // D_VAL DEBUG
    log_messages.printf(MSG_DEBUG,
                "Found %zu WUs that were validated by this host [#%d]\n", g_reply->dval_wu_input_files.size(), host_id);
    fflush(stderr);
...
// D_VAL
// changed to check if there is a D_VAL Workunit whose output files must be sent back to the server.
// Check if the client making the request is hosting those files. If so, ask for their upload.
//
void process_request(char* code_sign_key) {
    ...
    // BOINC-MR DEBUG
    log_messages.printf(MSG_CRITICAL, "[BOINC-MR] Inside process_request()\n");
    ...
    // D_VAL DEBUG
    log_messages.printf(MSG_DEBUG,
                            "Calling d_val_check_host_requested_files. g_reply->host.id: %d | initial_host.ID: %d\n", g_reply->host.id, initial_host.id
                        );

    // D_VAL
    // check if host making request holds required files
    d_val_check_host_requested_files(g_reply->host.id);

    // D_VAL DEBUG
    log_messages.printf(MSG_DEBUG, "Left d_val_check_host_requested_files.\n");
    fflush(stderr);
...
void handle_request(FILE* fin, FILE* fout, char* code_sign_key) {
    ...
    // BOINC-MR DEBUG
    log_messages.printf(MSG_CRITICAL, "[BOINC-MR] Inside handle_request(). Before calling sreq.parse(fin)");

    const char* p = sreq.parse(fin);
    // BOINC-MR DEBUG
    log_messages.printf(MSG_CRITICAL, "[BOINC-MR] Inside handle_request(). After calling sreq.parse(fin). Before process_request()");
    if (!p){
...
const char *BOINC_RCSID_2ac231f9de = "$Id: handle_request.cpp 19996 2009-12-21 00:58:58Z davea $";
