/**
//
// BOINC is free software; you can redistribute it and/or modify it
//

// A sample validator for work units that performed distributed validation (task validation by clients), that requires a majority of results to be
// bitwise identical.
// This is useful only if either
// 1) your application does no floating-point math, or
// 2) you use homogeneous redundancy

* Version info:
* => TODO - What to do with validated workunits (choose who will modify validate_state of original WUs that were being validated)
* - Same as bitwise validator
*
* v 0.1 - April 2014 - Fernando Costa

//
*/

#include "config.h"
#include "util.h"
#include "sched_util.h"
#include "sched_msgs.h"
#include "validate_util.h"
#include "md5_file.h"

using std::string;
using std::vector;

struct FILE_CKSUM_LIST {
    vector<string> files;   // list of MD5s of files
    ~FILE_CKSUM_LIST(){}
};

bool files_match(FILE_CKSUM_LIST& f1, FILE_CKSUM_LIST& f2) {

    /// BOINC-MR DEBUG
    log_messages.printf(MSG_CRITICAL,
        "[BOINC-MR DEBUG] files_match: Number of files. f1: %d | f2: %d\n",
        f1.files.size(), f2.files.size()
    );
    /// BOINC-MR DEBUG
    if (f1.files.size() != f2.files.size()){
        log_messages.printf(MSG_CRITICAL,
            "[BOINC-MR DEBUG] files_match: different number of files. f1: %d | f2: %d\n",
            f1.files.size(), f2.files.size()
        );
    }

    /// BOINC-MR DEBUG
    for (unsigned int k=0; k<f1.files.size(); k++) {
        if (f1.files[k] != f2.files[k]){
            log_messages.printf(MSG_CRITICAL,
            "[BOINC-MR DEBUG] files_match: different hash for file #%d. f1: %s | f2: %s\n",
            k, f1.files[k].c_str(), f2.files[k].c_str()
        );
        }
    }


    if (f1.files.size() != f2.files.size()) return false;
    for (unsigned int i=0; i<f1.files.size(); i++) {
        if (f1.files[i] != f2.files[i]) return false;
    }
    return true;
}

int init_result(RESULT& result, void*& data) {
    int retval;
    FILE_CKSUM_LIST* fcl = new FILE_CKSUM_LIST;
    vector<FILE_INFO> files;
    char md5_buf[MD5_LEN];
    double nbytes;

    retval = get_output_file_infos(result, files);
    if (retval) {
        log_messages.printf(MSG_CRITICAL,
            "[RESULT#%d %s] check_set: can't get output filenames\n",
            result.id, result.name
        );
        return retval;
    }

    for (unsigned int i=0; i<files.size(); i++) {
        FILE_INFO& fi = files[i];
        if (fi.no_validate) continue;
        retval = md5_file(fi.path.c_str(), md5_buf, nbytes);
        if (retval) {
            if (fi.optional) {
                strcpy(md5_buf, "");
                    // indicate file is missing; not the same as md5("")
            } else {
                log_messages.printf(MSG_CRITICAL,
                    "[RESULT#%d %s] Couldn't open %s\n",
                    result.id, result.name, fi.path.c_str()
                );
                return retval;
            }
        }
        fcl->files.push_back(string(md5_buf));
    }
    data = (void*) fcl;
    return 0;
}

int compare_results(
    RESULT & /*r1*/, void* data1,
    RESULT const& /*r2*/, void* data2,
    bool& match
) {
    FILE_CKSUM_LIST* f1 = (FILE_CKSUM_LIST*) data1;
    FILE_CKSUM_LIST* f2 = (FILE_CKSUM_LIST*) data2;

    match = files_match(*f1, *f2);
    return 0;
}

int cleanup_result(RESULT const& /*result*/, void* data) {
    delete (FILE_CKSUM_LIST*) data;
    return 0;
}

double compute_granted_credit(WORKUNIT& wu, vector<RESULT>& results) {
    return median_mean_credit(wu, results);
}
