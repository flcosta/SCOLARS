...
#include "backend_lib.h"    // required to update xml_signature whenever a non-MR user tries to request a hashed result (not able to host the map output)
                            /// BOINC-MR TODO: alternative - create two WUs, for the same MR job, one for MR users, another for normal hosts
                            /// MR job-related verifications (number of available maps) are done together.
                            /// Problem: impossible to compare output of two different WUs...

#ifdef _USING_FCGI_
#include "boinc_fcgi.h"
#endif

// BOINC-MR - required to modify results sent to clients who are unable to host map outputs
// deprecated
...
// BOINC-MR
// insert "text" right after ALL occurences of "after" in the given buffer
//
int insert_after_all(char* buffer, const char* after, const char* text) {
    char* p;
    /// BOINC-MR TODO : cleanup - hack to allow larger wu.xml_doc
    ///char temp[BLOB_SIZE];
    char temp[MEDIUM_BLOB_SIZE];

    if (strlen(buffer) + strlen(text) > MEDIUM_BLOB_SIZE-1) {
        log_messages.printf(MSG_CRITICAL,
            "insert_after_all: overflow: %d %d\n", strlen(buffer), strlen(text)
        );
...
// add ips to wu.xml doc
//
int add_ips_to_xml_doc(std::vector<std::string>& ips, std::string wu_name, WORKUNIT& wu){
    std::string partial_map_out_file;
    char* p, *q;
    /// BOINC-MR TODO : cleanup - hack for larger wu.xml_doc
    ///char temp[BLOB_SIZE];
    char temp[MEDIUM_BLOB_SIZE];
    //char tag[16];
    std::string mapper_addresses;
    //int retval = 0;

    // BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "sched_send: Inside add_ips_to_xml_doc()\n");
    fflush(stderr);

    // add all ips in ips_to_add to file info corresponding to cur_wu_name
    // IMPORTANT: must add "_" to avoid errors when looking for single digits. e.g.: "wu_3" search result: wu_3_*; wu_30_*; wu_31_*; etc. "wu_3_" returns only "wu_3_*"
    //sprintfn(partial_map_out_file, "<name>%s", cur_wu_name);
    partial_map_out_file = "<name>"+wu_name+"_";
    p = strstr(wu.xml_doc, partial_map_out_file.c_str());
    if (!p) {
        log_messages.printf(MSG_CRITICAL,
            "add_ips_to_xml_doc: %s not found in wu.xml_doc: %s\n", partial_map_out_file.c_str(), wu.xml_doc
        );
        return ERR_NULL;
    }

    // BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "add_ips_to_xml_doc: found the string: %s\n", partial_map_out_file.c_str());
    fflush(stderr);

    // append to file
    //mapper_addresses = "";
    mapper_addresses = "    <mr_mappers>";
    /// BOINC-MR TODO - randomize IP addresses (to achieve better load balancing)
    random_shuffle (ips.begin(), ips.end());
    // mapper_addresses = "<mr_mappers>";
    for (int j=0; j<ips.size(); j++){
        mapper_addresses += ips[j];
        //mapper_addresses += "    <mr_mapper>"+ips[j]+"</mr_mapper>\n";
        // in last iteration add ending tag, otherwise add a blank space
        if (j==(ips.size()-1))
            mapper_addresses += "</mr_mappers>\n";
        else
            mapper_addresses += " ";
    ...
    log_messages.printf(MSG_DEBUG, "add_ips_to_xml_doc: inserting ip addresses: %s\n", mapper_addresses.c_str());
    fflush(stderr);

    // add addresses after "</name>" - start searching from p (beginning of <name> tag)
    ///retval = insert_after(p, "</name>\n", mapper_addresses.c_str());
    /// insert_after not used because we want to start searching at "p", not from the beginning of wu.xml_doc

    if (strlen(wu.xml_doc) + strlen(mapper_addresses.c_str()) > MEDIUM_BLOB_SIZE-1) {
        log_messages.printf(MSG_CRITICAL,
            "add_ips_to_xml_doc: overflow: %zu %zu\n", strlen(wu.xml_doc), strlen(mapper_addresses.c_str())
        );
        return ERR_BUFFER_OVERFLOW;
    }

    //sprintf(tag, "</name>\n");
    // start searching at p (after <name>WU_NAME)
    q = strstr(p, "</name>\n");
    if (!q) {
        log_messages.printf(MSG_CRITICAL,
            "add_ips_to_xml_doc: \"</name\"> not found in %s\n", p
        );
        return ERR_NULL;
    }
    q += strlen("</name>\n");
    strcpy(temp, q);
    strcpy(q, mapper_addresses.c_str());
    strcat(q, temp);
    ...
    /// BOINC-MR DEPRECATED
    /// add addresses before <name> tag
    /*if (strlen(wu.xml_doc) + strlen(mapper_addresses.c_str()) > BLOB_SIZE-1) {
        log_messages.printf(MSG_CRITICAL, "add_ips_to_xml_doc: overflow: %d %d\n",
        strlen(wu.xml_doc), strlen(mapper_addresses.c_str()));
        return ERR_BUFFER_OVERFLOW;
    }
    strcpy(temp, p);
    strcpy(p, mapper_addresses.c_str());
    strcat(p, temp);
    p += strlen(mapper_addresses.c_str());
    */
...
// Insert into xml doc addresses of mappers hosting map outpus for the reduce workunit "wu"
//
int insert_wu_map_hosts(std::vector<std::string>& mapper_ips, std::vector<std::string>& wu_names, WORKUNIT& wu){
    char buf[BLOB_SIZE];
    int retval;
    //int map_wu_index

    // BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "sched_send: Inside insert_wu_map_hosts()\n");
    fflush(stderr);


    ///char partial_map_out_file[300];
    size_t found;
    //int len;
    std::vector<std::string> ips_to_add;   // hosts that share the same file (executed same map workunit)
    std::string cur_wu_name;

    // if there is only one available Map output (only one client executed map workunit and hosts the output)
    if (wu_names.size() == 1){
        cur_wu_name = wu_names[0];
        ips_to_add.push_back(mapper_ips[0]);
    }
    // else there are several work units available
    // go through each result and find out its workunit index ($WUNAME_$WUINDEX_$RESULTINDEX)
    else{
        std::vector<std::string>::iterator wu_name = wu_names.begin();
        std::vector<std::string>::iterator ip = mapper_ips.begin();
        // save first element, and compare sequentially, to save all IPs that are hosting the same map output files (same workunit name)
        cur_wu_name = *wu_name;
        ips_to_add.push_back(*ip);

        // BOINC-MR DEBUG
        log_messages.printf(MSG_DEBUG, "insert_wu_map_hosts: first wu name: %s | IP: %s\n", cur_wu_name.c_str(), (*ip).c_str());
        fflush(stderr);

        wu_name++;
        ip++;
        for (; wu_name!=wu_names.end(); wu_name++, ip++) {


            // BOINC-MR DEBUG
            log_messages.printf(MSG_DEBUG, "insert_wu_map_hosts: previous wu name: %s || current: %s | IP: %s\n",
                                cur_wu_name.c_str(), (*wu_name).c_str(), (*ip).c_str());
            fflush(stderr);

            // check if next wu name is equal to previous. If the wu has the same name, save IP on array since it is hosting the same file.
            if (cur_wu_name.compare(*wu_name) == 0){
                ips_to_add.push_back(*ip);
            }
            // If it is a different wu, add existing array to wu xml doc, and start a new IP list for this wu
            else{

                // BOINC-MR DEBUG
                log_messages.printf(MSG_DEBUG, "insert_wu_map_hosts: previous wu name DIFFERENT: %s [VS.] %s\n",
                                    cur_wu_name.c_str(), (*wu_name).c_str());
                fflush(stderr);

                // add ips wu to xml doc. Reset ip array and update current wu name
                retval = add_ips_to_xml_doc(ips_to_add, cur_wu_name, wu);
                // error while adding ips to xml doc
                if (retval){
                    log_messages.printf(MSG_DEBUG, "insert_wu_map_hosts: error adding ips to wu.xml_doc. WU name: %s\n", cur_wu_name.c_str());
                    return retval;
                }
                ips_to_add.clear();
                cur_wu_name = *wu_name;
    ...
    // reached the end of array of mapper IP addresses returned from the DB.
    // Add last vector of ips_to_add to xml doc (was not added within for loop) OR
    // a single address was returned, which has not been added either
    retval = add_ips_to_xml_doc(ips_to_add, cur_wu_name, wu);
    if (retval){
        log_messages.printf(MSG_DEBUG, "insert_wu_map_hosts: error adding ips to wu.xml_doc. WU name: %s", cur_wu_name.c_str());
        return retval;
    }
    ...
    /// BOINC-MR DEPRECATED
    /*sprintf(buf, "<mr_mappers_addresses>\n");

    // add each address to WU xml doc
    std::vector<std::string>::iterator i=ips.begin();
    for (;i!=ips.end();i++) {
        sprintf(mapper, "  <mr_mapper>%s</mr_mapper>\n", (*i).c_str());
        // check if value can be added to buffer. If not, return error - buffer overflow
        if (strlen(mapper) + strlen(buf) > BLOB_SIZE-1){
            log_messages.printf(MSG_CRITICAL,
                "mr_insert_mappers_addr: overflow: %d %d\n", strlen(mapper), strlen(buf)
            );
            return ERR_BUFFER_OVERFLOW;
        }
        strcat(buf, mapper);
    ...
    // BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "mr_insert_mappers_addr: Mapper IP address [%s] saved in buf\n", mapper);
    fflush(stderr);

    }
    sprintf(mapper, "</mr_mappers_addresses>\n");
    if (strlen(mapper) + strlen(buf) > BLOB_SIZE-1){
        log_messages.printf(MSG_CRITICAL,
            "mr_insert_mappers_addr: overflow: %d %d\n", strlen(mapper), strlen(buf)
        );
    ...
    // BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "mr_insert_mappers_addr: adding %s\n to wu.xml_doc\n", buf);
    fflush(stderr);

    ///retval = insert_after_all(wu.xml_doc, "<file_info>\n", buf);
    /// Problem - too much info inserted into xml_doc
    //retval = insert_after(wu.xml_doc, "<workunit>\n", buf);
    /// BOINC-MR DEPRECATED - wrong (insert each host after its corresponding <file_info>
    */
}


// BOINC-MR
// Insert Mappers IP addresses in Reduce WU XML
// result_names are required to identify which file each client is hosting
int mr_insert_mappers_addr(WORKUNIT& wu, int jobid) {
    std::vector<std::string> ips, wu_names;
    int retval;
    //int rem_bytes = BLOB_SIZE;              // number of remaining space on buffer "buf"


    // BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "sched_send: Inside mr_insert_mappers_addr()\n");
    fflush(stderr);


    retval = enumerate_ip_mappers(ips, wu_names, jobid);
    if (retval) {
        log_messages.printf(MSG_CRITICAL,
            "mr_insert_mappers_addr: can't find IP addresses of hosts that returned Map results [MR job id #%d]: %d\n", jobid, retval
        );
        return retval;
    }
    ...
    for (int i=0; i<ips.size(); i++){
        temp += "IP: "+ips[i] + " | Wu name: " + wu_names[i]+"\n";
    }
    log_messages.printf(MSG_DEBUG, "Content returned by SQL Query:\n%s", temp.c_str());
    fflush(stderr);

    // only call function if there are any mappers available
    if (ips.size() > 0){
        retval = insert_wu_map_hosts(ips, wu_names, wu);
        if (retval) return retval;
    }
...
int add_wu_to_reply(
    ...
    // BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "Checking if need to call mr_insert_mappers_addr(). MR job id: %d | MR app type: %d \n", wu.mr_job_id, wu.mr_app_type);
    fflush(stderr);

    // If this is a Reduce Work Unit, insert IP addresses of users hosting Map outputs
    if (wu.mr_job_id > 0 && wu.mr_app_type == MR_APP_TYPE_REDUCE){

        // BOINC-MR DEBUG
        log_messages.printf(MSG_DEBUG, "Calling mr_insert_mappers_addr()...\n");
        fflush(stderr);
        ...
        // BOINC-MR DEBUG
            ...
            log_messages.printf(MSG_DEBUG, "add_wu_to_reply: WU XML doc [after adding mapper addr]: %s\n", wu3.xml_doc);
    ...
    // BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "add_wu_to_reply: After adding to workunit unique\n");
    fflush(stderr);
...
// return true if this result is supposed to return hashes of outputs, instead of output files themselves
// - look for <mr_hash/> tag
//
int is_result_hashed(RESULT& result){
    char buf[256];
    char* p;

    sprintf(buf, "<mr_hash/>\n");
    p = strstr(result.xml_doc_in, buf);
    if (!p) {
        log_messages.printf(MSG_DEBUG,
            "is_result_hashed: Result [ID%d] is not hashed\n", result.id
        );
        return false;
    }
    return true;
...
// insert <upload_when_present/> tag in first half of output files
// When that tag is encountered, we have reached the hashes and can stop
int insert_upload_tags(RESULT& result) {

    char* p;
    /// BOINC-MR TODO : cleanup - hack to allow larger result.xml_doc_in
    ///char temp[BLOB_SIZE];
    char temp[MEDIUM_BLOB_SIZE];
    // tag to insert
    std::string upload_tag = "    <upload_when_present/>\n";
    bool found_existing_tag = false;
    bool found_nothing = true;
    p = result.xml_doc_in;
    //std::string str_cmp = "<upload_when_present/>";
    char buf[256];
    std::string prev_tag = "<generated_locally/>\n";

    while (p = strstr(p, prev_tag.c_str())){
        // failsafe - should not be needed
        if (found_existing_tag)
            break;

        p += strlen(prev_tag.c_str());

        // check if this is a hash file - already has the <upload_when_present/> tag
        // save the space corresponding to the expected hash, and check if it matches
        strncpy(buf, p, strlen(upload_tag.c_str()));
        if (strcmp(buf, upload_tag.c_str()) == 0){
            found_existing_tag = true;
            break;
        }

        // this is not a hash, so add the upload tag here
        if (strlen(result.xml_doc_in) + strlen(upload_tag.c_str()) > MEDIUM_BLOB_SIZE-1) {
            log_messages.printf(MSG_CRITICAL,
                "insert_upload_tags: overflow: %d %d\n", strlen(result.xml_doc_in), strlen(upload_tag.c_str())
            );
            return ERR_BUFFER_OVERFLOW;
        }
        strcpy(temp, p);
        strcpy(p, upload_tag.c_str());
        strcat(p, temp);
        p += strlen(upload_tag.c_str());
        found_nothing = false;
    }
    if (!p && found_nothing) {
    //if (!p) {
        log_messages.printf(MSG_CRITICAL,
            "insert_upload_tags: %s not found in %s\n", prev_tag.c_str(), p
        );
        return ERR_NULL;
    }
    return 0;
...
int add_result_to_reply(
    ...
    /// BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "add_result_to_reply: Added wu to reply successfully. Locality scheduling?: %d\n", locality_scheduling);
    fflush(stderr);
    ...
    /// BOINC-MR TODO - change report_deadline to wu.mr_delay_map_bound if
    /// this (Reduce) result was created while inputs were still unavailable (Map outputs not returned yet)
    /// BOINC-MR TODO - update DB here or before and only call this funtion when you have ALL inputs and are
    /// effectively sending this result to the client (instead of just updating him on new inputs)
    //
    //if (result.server_state = RESULT_SERVER_STATE_MR_INC_INPUTS){
    //    // still call mark_as_sent to update DB, but old_server_state is the same as current one,
    //    // unless this time we are really sending it (in that case, server_state = UNSENT) so we wouldn't have come in here
    //    result.report_deadline = result.sent_time + wu.mr_delay_map_bound;
    //} else (result.server_state != RESULT_SERVER_STATE_IN_PROGRESS) { (...)
    ...
    /// BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "add_result_to_reply: estimated duration. Now updating wu transition time");
    fflush(stderr);
    ...
    /// BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "add_result_to_reply: Inserted name tags successfully\n");
    fflush(stderr);
    ...
    // If this is a Map Work Unit, with hashes being returned (mr_hashed), and currently being submitted to a client who is unable to host the output files themselves
    if (wu.mr_job_id > 0 && wu.mr_app_type == MR_APP_TYPE_MAP){
        /// BOINC-MR TODO: check if this does not cause problems. mapreduce_jobs only used by transitioner... what if it crashes?
        /// look for <mr_hash/> tag in result.xml_doc_in instead
        //MAPREDUCE_JOB *mr_j;
        //mr_j = mapreduce_jobs.mr_get_job(wu.mr_job_id);
        //wu.mr_job_id

        // check if
        // - this result is supposed to return hashes of map outputs - look for <mr_hash/> tag in xml_doc_in;
        // - the client who requested work is unable to host these map outputs
        /// BOINC-MR TODO: scenario - normal client requests hashed map task. change xml_doc_in to accomodate the return of output files
        /// invalid signature problems with cliens when this is inserted - check with boinc-mr for problem
        /*if (is_result_hashed(result) && !g_request->mr_client){
            // have the client upload the output file itself back to server, by adding the tag <upload_when_present/>, after <generated_locally/>
            retval = insert_upload_tags(result);
            if (retval){
                log_messages.printf(MSG_CRITICAL,
                "Error inserting <upload_when_present/> tag to Result [ID%d]\n", ((RESULT&)result).id
                );
            }
            // update the xml_signature, since we have changed the xml_doc_in
            else{
                ...
                if (retval) {
                    log_messages.printf(MSG_CRITICAL, "add_result_to_reply() can't read key\n");
                    exit(1);
                }
                /// BOINC-MR DEBUG
                log_messages.printf(MSG_CRITICAL, "After reading key from %s/upload_private, before add_signatures\n",config.key_dir);

                char temp[MEDIUM_BLOB_SIZE];
                //retval = add_signatures(((RESULT&)result).xml_doc_in, key);
                strncpy(temp, ((RESULT&)result).xml_doc_in, sizeof(temp)-1);
                // if there's an error, the result's xml_doc_in remains unchanged

                // delete the previous signature - use loop to remove signature from all files
                while (remove_element(temp, "<xml_signature>", "</xml_signature>")) {
                    continue;
                }

                retval = add_signatures(temp, key);
                if (retval){
                    log_messages.printf(MSG_CRITICAL, "Error updating the xml_signature of result [ID#%d]\n", ((RESULT&)result).id);
                }
                // update xml_doc_in if there were no errors
                else{
                    strncpy(((RESULT&)result).xml_doc_in, temp, sizeof(((RESULT&)result).xml_doc_in));
                }
            }
        }*/
    }
    ...
    /// BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG, "add_result_to_reply: Returning 0! Success!\n");
    fflush(stderr);
...
const char *BOINC_RCSID_32dcd335e7 = "$Id: sched_send.cpp 19273 2009-10-06 20:45:09Z davea $";
