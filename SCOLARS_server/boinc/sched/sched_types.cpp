...
const char* SCHEDULER_REQUEST::parse(FILE* fin) {
    ...
    // BOINC-MR
    mr_client = false;          // whether this is a request from a client able to host/upload map outputs
    ...
    while (fgets(buf, sizeof(buf), fin)) {
        ...
        // BOINC-MR: check if this is a request from a MR client (client able to host and upload map output from MapReduce job)
        if (match_tag(buf, "<mr_client/>")){
            mr_client = true;
            continue;
        }
...
int SCHEDULER_REPLY::write(FILE* fout, SCHEDULER_REQUEST& sreq) {
    ...
    log_messages.printf(MSG_DEBUG,
        "Scheduler::write. About to write %zu return_output and %zu return_input requests.\n",
        dval_result_output_files.size(), dval_wu_input_files.size());
    fflush(stderr);

    // D_VAL
    // ask clients to return output files for all WUs whose names are in 'dval_result_output_files'
    for (i=0; i<dval_result_output_files.size(); i++){
        fprintf(fout,
            "<return_output>%s</return_output>\n",
            dval_result_output_files[i].c_str()
        );


        // D_VAL DEBUG
        log_messages.printf(MSG_DEBUG,
            "Scheduler::write. Just wrote: <return_output>%s</return_output>\n",
            dval_result_output_files[i].c_str());
        fflush(stderr);

    }
    // ask clients to return input files for all WUs whose names are in 'dval_wu_input_files'
    for (i=0; i<dval_wu_input_files.size(); i++){
        fprintf(fout,
            "<return_input>%s</return_input>\n",
            dval_wu_input_files[i].c_str()
        );

        // D_VAL DEBUG
        log_messages.printf(MSG_DEBUG,
            "Scheduler::write. Just wrote: <return_input>%s</return_input>\n",
            dval_wu_input_files[i].c_str());
        fflush(stderr);
    }


    // if requesting file(s), the server must also provide de upload url
    if (dval_result_output_files.size() > 0 || dval_wu_input_files.size() > 0){
        fprintf(fout,
            "<dval_req_url>%s</dval_req_url>\n",
            config.upload_url
        );
    }
    ...
    /// BOINC-MR DEBUG
    log_messages.printf(MSG_DEBUG,
        "Scheduler successfully exiting SCHEDULER_REPLY::write()");
...
int RESULT::parse_from_client(FILE* fin) {
    ...
    while (fgets(buf, sizeof(buf), fin)) {
        ...
        if (match_tag(buf, "<map/>")){
            mr_job_type = MR_MAP_RESULT;
            continue;
        }
        if (match_tag(buf, "<reduce/>")){
            mr_job_type = MR_REDUCE_RESULT;
            continue;
        }
...
const char *BOINC_RCSID_ea659117b3 = "$Id: sched_types.cpp 19002 2009-09-03 20:26:31Z davea $";
