...
int main(int argc, char** argv) {
    ...
    // BOINC-MR DEBUG
    //log_messages.printf(MSG_CRITICAL, "[BOINC-MR] Inside sched_main-main(). Before mapreduce_jobs.init()");
    //fflush(stderr);

    // BOINC-MR
    /// BOINC-MR TODO : parse mr_jobs.xml file and initialize variables
    ...
    // BOINC-MR DEBUG
    //log_messages.printf(MSG_CRITICAL, "[BOINC-MR] Inside sched_main-main(). After init(). Before mapreduce_jobs.parse()");
    //fflush(stderr);
    ...
    // D_VAL DEBUG
//    log_messages.printf(MSG_CRITICAL, "[D_VAL] Inside Scheduler-main(). Before dist_val_apps.init()\n");
//    fflush(stderr);
//
//    dist_val_apps.init();
//    // D_VAL DEBUG
//    log_messages.printf(MSG_CRITICAL, "[D_VAL] Inside Scheduler-main(). After init(). Before dist_val_apps.parse()\n");
//    fflush(stderr);
...
//    fflush(stderr);
    ...
    if (use_files) {
        ...
        // BOINC-MR DEBUG
        log_messages.printf(MSG_CRITICAL, "[BOINC-MR] Inside sched_main-main(). Calling handle_request()");
        handle_request(stdin, stdout, code_sign_key);
    }
done:
#ifdef _USING_FCGI_
...
const char *BOINC_RCSID_0ebdf5d770 = "$Id: sched_main.cpp 19288 2009-10-12 16:44:26Z davea $";
