...
int handle_results() {
    ...
    for (i=0; i<g_request->results.size(); i++) {
        ...
        if ((srip->client_state == RESULT_FILES_UPLOADED) && (srip->exit_status == 0)) {
            ...
            // BOINC-MR
            // By default, the Result's outputs are not being served by client
            srip->user_hosting = 0;
            // Result has been accepted so check if it is an MR job and a Map task and if so,
            // set user_hosting to TRUE (1) in result, to be updated later in DB
            // Check on xml_doc_in - set by admin/app dev on Result Template (do not depend on user output from xml_doc_out)
            if (match_tag(srip->xml_doc_in, "<mapreduce>") && match_tag(srip->xml_doc_in, "<map_output/>")){

                // BOINC-MR DEBUG
                log_messages.printf(MSG_CRITICAL, "[BOINC-MR] handle_results() - Received Map Result from Host #%d\n", g_reply->host.id);
                //log_messages.printf(MSG_CRITICAL, "XML doc in:\n%s", srip->xml_doc_in);


                //if (strstr(rp->xml_doc_out, "<mr_file_uploading/>"))
                  //  log_messages.printf(MSG_CRITICAL, "***** [BOINC-MR] **** TURNS OUT *** handle_results() - Host #%d is hosting "
                    //                    "Map output from result #%d ******\n", g_reply->host.id, srip->id);;

                /// BOINC-MR TODO : if more complex parsing is needed. use strtok [int result_delete_files(RESULT& result)]:
                //p = strtok(buf,"\n");
                // Check if file is being uploaded by the user
                if (match_tag(rp->xml_doc_out, "<mr_file_uploading/>")){
                    srip->user_hosting = 1;
                    // BOINC-MR DEBUG
                    log_messages.printf(MSG_CRITICAL, "[BOINC-MR] handle_results() - Host #%d is hosting "
                                        "Map output from result #%d\n", g_reply->host.id, srip->id);
                }
                /// BOINC-MR TODO : change this check to client [xml_doc_out] - if client is able to host the output,
                /// add <mr_hash/> to its corresponding hash output (that is returned instead of output)
                /// BOINC-MR TODO : handle error condition - client is unable to host output, so return both hash AND output and
                /// handle it at this end (copy only those files that were returned)
                // the <mr_hash/> tag is added before the client is sent the result. It does not mean it is serving the file.
                else if(match_tag(rp->xml_doc_out, "<mr_hash/>")){
                // else if(match_tag(srip->xml_doc_in, "<mr_hash/>")){
                    srip->user_hosting = 1;
                    // BOINC-MR DEBUG
                    log_messages.printf(MSG_CRITICAL, "[BOINC-MR] handle_results() - Host #%d is hosting "
                                        "Map output from result #%d\n", g_reply->host.id, srip->id);
                    ...
                    log_messages.printf(MSG_NORMAL, "[BOINC-MR] handle_results() - Map output from result #%d is "
                                        "not being served/uploaded by Host #%d. XML_DOC_OUT: %s\n", srip->id, g_reply->host.id, rp->xml_doc_out);
            }
            else
                log_messages.printf(MSG_DEBUG, "[BOINC-MR] handle_results() - Result #%d is not Map task from "
                                    "MapReduce job. XML_DOC_IN: %s\n", srip->id, srip->xml_doc_in);

            /// BOINC-MR DEPRECATED
            /*
            //add this DB_HOST to that job's mappers (who are serving map outputs)
            // Cannot do this in result_handler loop because we need to check result->is_mapreduce
            /// BOINC-MR TODO : check which files user is uploading. Version 1: Either all or no map outputs are being served
            if (rp->is_mapreduce && rp->mr_job_id && rp->mr_job_type == MR_MAP_RESULT){

                // BOINC-MR DEBUG
                log_messages.printf(MSG_CRITICAL, "[BOINC-MR] handle_results() - Received Map Result from Host #%d", g_reply->host.id);

                // check if user is serving output files
                //if (mapreduce_jobs.mr_check_file_served(rp->xml_doc_out, rp->mr_job_id)){
                if (match_tag(rp->xml_doc_out, "<MR_FILE_UPLOADING/>")){
                    retval = mapreduce_jobs.mr_add_host_job(g_reply->host, rp->mr_job_id);
                    if (retval){
                        log_messages.printf(MSG_INFO, "[BOINC-MR] handle_results() - Error adding [host #%d] to list of serving mappers.", g_reply->host.id);
                    }
                    // BOINC-MR DEBUG
                    log_messages.printf(MSG_CRITICAL, "[BOINC-MR] handle_results() - Host #%d is hosting Map output from result #%d", g_reply->host.id, rp->id);
                }
                //BOINC-MR
                else
                    log_messages.printf(MSG_CRITICAL, "[BOINC-MR] handle_results() - Map output is not being served/uploaded!!");
            }
            // BOINC-MR DEBUG
            else
                log_messages.printf(MSG_CRITICAL, "[BOINC-MR] handle_results() - Result #%d is not Map task from MapReduce job!!", rp->id);
    ...
    // BOINC-MR DEBUG
    log_messages.printf(MSG_CRITICAL, "[BOINC-MR] handle_results() - Calling update_workunits()");
