...
// Distributed validation errors
#define ERR_DIST_VAL_UNIQUE_ALT_NAMES -800          // could not create unique alternative names, by modifying result template (result.xml_doc_in)
#define DVAL_ERR_MISMATCH_NUM_RESULTS -801          // while reading output file from validator WU, realized that there was a wrong number of lines (results validated) found, which did not match DB
