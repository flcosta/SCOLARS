...
int main(int argc, const char** argv) {
    ...
    while (i < argc) {
        if (arg(argv, i, "appname")) {
            ...
            assign_id = atoi(argv[++i]);
        // D_VAL
        // alternative name for Workunit created in shell script, so not necessary to use the create_unique_alt_name() function
        } else if (arg(argv, i, "alt_name")) {
            strcpy(wu.alt_name, argv[++i]);
        } else {
            if (!strncmp("-", argv[i], 1)) {
...
const char *BOINC_RCSID_3865dbbf46 = "$Id: create_work.cpp 19054 2009-09-16 03:10:22Z davea $";
