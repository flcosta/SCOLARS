...
// D_VAL
#define OBFUSC_ALT_NAME_NUM_CHARS 15        // number of characters of WU's alternative name (used for obfuscation purposes)
...
// D_VAL
int create_unique_alt_name(char*, int);
        // called by transitioner when processing the workunit template. If a <dist_val/> or <obfusc/> tag is found, an alternative name must be created
        // for this workunit. The new name is returned by function, while the string length is defined by num_chars.
