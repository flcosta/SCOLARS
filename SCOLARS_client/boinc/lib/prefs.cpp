...
// BOINC-MR
#include "client_types.h"       // MR_DEFAULT_PORT
...
void GLOBAL_PREFS::defaults() {
    ...
    // BOINC-MR
    // port to listen for incoming connections to upload (map) outputs [DEFAULT: 8000]
    //mr_output_port = 8000;
...
// set port used in inter-client transfers
void GLOBAL_PREFS::mr_set_output_port(int port){
    mr_output_port = port;
}
...
int GLOBAL_PREFS::parse_override(
    ...
    double dtemp;
    // BOINC-MR (for int mr_output_port)
    int itemp;
    ...
    while (!xp.get(tag, sizeof(tag), is_tag, attrs, sizeof(attrs))) {
        ...
        // BOINC-MR DEBUG
        printf("Mar 2010 [---] BOINC-MR Debug: Inside GLOBAL_PREFS::parse_override - parsing 'mr_output_port'\n");

        /// BOINC-MR TODO
        // Validate chosen port (mr_output_port > 1024, < 65535)
        if (xp.parse_int(tag, "mr_output_port", itemp)){
            // further validation needed? Or just warning if inside "unsecure" range
            if (itemp>0){
                mr_output_port = itemp;
                mask.mr_output_port = true;
            }
            continue;
...
const char *BOINC_RCSID_3fb442bb02 = "$Id: prefs.cpp 20786 2010-03-04 04:16:00Z davea $";
