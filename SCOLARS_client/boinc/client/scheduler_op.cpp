...
int SCHEDULER_OP::init_op_project(PROJECT* p, int r) {
    ...
    // BOINC-MR DEBUG
    //printf("Entered SCHEDULER_OP::init_op_project\n");
    ...
    // BOINC-MR DEBUG
    //printf("Leaving SCHEDULER_OP::init_op_project\n");
    //msg_printf(p, MSG_INFO, "Leaving SCHEDULER_OP::init_op_project");
...
int SCHEDULER_OP::start_rpc(PROJECT* p) {
    ...
    // BOINC-MR DEBUG
    //printf("Entered SCHEDULER_OP::start_rpc\n");
    ...
    // BOINC-MR DEBUG
    //printf("Exiting SCHEDULER_OP::start_rpc\n");
...
bool SCHEDULER_OP::poll() {
    ...
    // BOINC-MR DEBUG
    //printf("Entered SCHEDULER_OP::poll\n");
    ...
    case SCHEDULER_OP_STATE_RPC:
        ...
        if (http_op.http_op_state == HTTP_STATE_DONE) {
            ...
            if (http_op.http_op_retval) {
                ...
                // BOINC-MR DEBUG
                printf("Inside SCHEDULER_OP::poll - left gstate.handle_scheduler_reply. Retval: %d\n",(int)retval);
                switch (retval) {
                case 0:
                    break;
            ...
            // BOINC-MR DEBUG
            //printf("Leaving SCHEDULER_OP::poll . Return TRUE\n");
...
int SCHEDULER_REPLY::parse(FILE* in, PROJECT* project) {
    ...
    // D_VAL
    dval_req_inputs.clear();
    dval_req_outputs.clear();
    std::string dval_req_file;
    //dval_req_url = "";
    ...
    while (fgets(buf, 256, in)) {
        ...
        // BOINC-MR DEBUG
        //msg_printf(0, MSG_INFO, "[BOINC-MR DEBUG] SCHEDULER_REPLY::parse() Parsing: %s\n", buf);
        ...
        if (match_tag(buf, "</scheduler_reply>")) {
            ...
            // D_VAL
            // sanity check. If client was given a list of required files (input or output), but no URL to upload them to, clear the vectors
            if ((dval_req_outputs.size() > 0 || dval_req_inputs.size() > 0) && dval_req_url.empty()){
                msg_printf(project, MSG_INTERNAL_ERROR,
                    "Server required %zu input files and %zu output files, but no upload url was given (tag <dval_req_url not found).\n",
                    dval_req_inputs.size(), dval_req_outputs.size()
                );
            }
            ...
            if (retval) {
                ...
                // BOINC-MR DEBUG
                printf("SCHED_REPLY::parse() Found new WU, adding it: %s\n", wu.name);
                fflush(stdout);
            ...
            continue;
        // D_VAL
        // save info on all required files - fill up vector with WU names
        }else if(parse_str(buf, "<return_input>", dval_req_file)) {

            // D_VAL DEBUG
            msg_printf(project, MSG_INFO,
                "[D_VAL] SCHEDULER_REPLY::parse(): Found <return_input> for wu name: %s\n", dval_req_file.c_str()
            );

            dval_req_inputs.push_back(dval_req_file);
            continue;
        }else if(parse_str(buf, "<return_output>", dval_req_file)) {

            // D_VAL DEBUG
            msg_printf(project, MSG_INFO,
                "[D_VAL] SCHEDULER_REPLY::parse(): Found <return_output> for wu name: %s\n", dval_req_file.c_str()
            );

            dval_req_outputs.push_back(dval_req_file);
            continue;
        }else if(parse_str(buf, "<dval_req_url>", dval_req_url)) {

            // D_VAL DEBUG
            msg_printf(project, MSG_INFO,
                "[D_VAL] SCHEDULER_REPLY::parse(): Found <d_val_req_url>: %s\n", dval_req_url.c_str()
            );
...
const char *BOINC_RCSID_11c806525b = "$Id: scheduler_op.cpp 20772 2010-03-02 18:58:34Z davea $";
