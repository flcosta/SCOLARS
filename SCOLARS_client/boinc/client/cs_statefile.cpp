...
int CLIENT_STATE::parse_state_file() {
    ...
    while (fgets(buf, 256, f)) {
        ...
        if (match_tag(buf, "<workunit>")) {
            ...
            // D_VAL DEBUG
            printf("CLIENT_STATE::parse_state_file(). Found <workunit> and parsing it. calling link_workunit()\n");
            fflush(stdout);
...
int CLIENT_STATE::write_state_file() {
    // BOINC-MR DEBUG
    //printf("Entered CLIENT_STATE::write_state_file()\n");
    MFILE mf;
    ...
    for (attempt=1; attempt<=MAX_STATE_FILE_WRITE_ATTEMPTS; attempt++) {
        ...
        // BOINC-MR DEBUG
        //printf("CLIENT_STATE::write_state_file => Entering write_state(miof)\n");
    ...
    // BOINC-MR DEBUG
    //printf("Leaving CLIENT_STATE::write_state_file()\n");
...
int CLIENT_STATE::write_state(MIOFILE& f) {
    ...
    for (j=0; j<projects.size(); j++) {
        ...
        // BOINC-MR DEBUG
        //printf("CLIENT_STATE::write_state => wrote project [p->write_state(f)]. Retval: %s\n", retval);
        //printf("EXIT write_state 2 - retval: %i\n",retval);
        //printf("EXIT write_state 3 - retval: %s\n",retval);
        //msg_printf(NULL, MSG_INFO, "CLIENT_STATE::write_state => wrote project [p->write_state(f)]. Retval: %s", retval);
        //printf("EXIT write_state X\n");
    ...
    // BOINC-MR DEBUG
    //printf("CLIENT_STATE::write_state => writing active_tasks\n");
...
int CLIENT_STATE::write_state_file_if_needed() {
    // BOINC-MR DEBUG
    //msg_printf(NULL, MSG_INFO, "Entered write_state_file_if_needed()");
    int retval;
    if (client_state_dirty) {
        client_state_dirty = false;
        // BOINC-MR DEBUG
        //printf("CS::write:state_file_if_needed => Entering write_state_file()\n");
...
const char *BOINC_RCSID_375ec798cc = "$Id: cs_statefile.cpp 20296 2010-01-28 13:29:10Z davea $";
