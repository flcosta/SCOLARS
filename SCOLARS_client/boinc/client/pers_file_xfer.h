...
#include "mr_tasktracker.h"         // MR_CLIENT_XFER
...
// BOINC-MR
#define MR_CLIENT_PERS_GIVEUP   (SECONDS_PER_DAY*2)
    // give up on trying to download from clients if this time elapses
    // cannot be too long since Mappers will eventually delete it, and Reduce
#define MR_CLIENT_MAX_CONN_ATTEMPTS      100
    // give up on trying to download from Mapper clients after this number of tries
...
public:
    ...
    // BOINC-MR
    MR_CLIENT_XFER* mr_cxp;
    int mr_conn_attempts;
        // number of connection attempts to clients to download the file in this Pers file xfer
};
...
public:
    ...
    void set_mr_client_xfer_set(MR_CLIENT_XFER_SET* xfers){
        mr_client_xfers = xfers;
    };
};
