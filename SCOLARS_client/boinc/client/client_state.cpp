    ...
    get_project_list_op(&gui_http)
{
    ...
    // BOINC-MR
    //mr_file_server = new MR_FILE_SERVER;
    mr_client_connections = new MR_CLIENT_CONNECTION_SET();
    mr_client_xfers = new MR_CLIENT_XFER_SET();
    pers_file_xfers->set_mr_client_xfer_set(mr_client_xfers);
    mr_client_xfer_giveup_period = MR_CLIENT_PERS_GIVEUP;
    mr_max_client_conn_attempts = MR_CLIENT_MAX_CONN_ATTEMPTS;
}
...
bool CLIENT_STATE::poll_slow_events() {
    ...
    if (!network_suspended && suspend_reason != SUSPEND_REASON_BENCHMARKS) {
        ...
        /// BOINC-MR TODO
        // go through list of open sockets / serving files and check if any have gone over the time-out set by client
        //POLL_ACTION(mr_file_uploads       , mr_file_uploads->poll);
        POLL_ACTION(mr_handle_file_uploads, mr_handle_file_uploads); // go through FILE_INFOs and start hosting these files - add them to list of files to be uploaded (mr_file_upload_list)
        POLL_ACTION(mr_client_xfers       , mr_client_xfers->poll       );
    }
...
WORKUNIT* CLIENT_STATE::lookup_workunit(PROJECT* p, const char* name) {
    for (unsigned int i=0; i<workunits.size(); i++) {
        ...
        /// BOINC-MR DEBUG
        printf("Inside CLIENT_STATE::lookup_workunit. Workunit[%d]: %s.\n", i, wup->name);
        fflush(stdout);
        if (wup->project == p && !strcmp(name, wup->name)) return wup;
    }
    /// BOINC-MR DEBUG
    printf("Inside CLIENT_STATE::lookup_workunit. Did not find workunit with this name: %s.\n", name);
    fflush(stdout);
    return 0;
}
...
// BOINC-MR
// save this result's work unit name on output files (needed for upload of Map outputs)
/// Distributed Validation: D_VAL
// if this result is going to be validated by another client, save that information on its output files
int CLIENT_STATE::link_result(PROJECT* p, RESULT* rp) {
    WORKUNIT* wup;
    unsigned int i;
    int retval;

    printf("Inside [CLIENT_STATE::link_result] Calling lookup_workunit to find wu with name [rp->wu_name]: %s\n", rp->wu_name);
    fflush(stdout);
    ...
    for (i=0; i<rp->output_files.size(); i++) {
        ...
        if (retval) return retval;
        // BOINC-MR
        strcpy(rp->output_files[i].file_info->mr_wu_name, rp->wu_name);

        // D_VAL DEBUG
        /*printf("[CLIENT_STATE::link_result] rp->output_files[i].file_info->dist_val: %d\n",
               rp->output_files[i].file_info->dist_val);
        fflush(stdout);

        printf("[CLIENT_STATE::link_result] Workunit's alt_name [wup->alt_name]: %s\n", wup->alt_name.c_str());
        fflush(stdout);
        printf("[CLIENT_STATE::link_result] rp->output_files[i].file_info->alt_wu_name: %s\n", rp->output_files[i].file_info->alt_wu_name.c_str());
        fflush(stdout);
        */

        // D_VAL
        // copy workunit alternative file name to file_info (default: "")
        //strcpy(rp->output_files[i].file_info->alt_wu_name, wup->alt_name);
        rp->output_files[i].file_info->alt_wu_name =  wup->alt_name;

        // D_VAL DEBUG
        //printf("[CLIENT_STATE::link_result] Setting dist_val of file [%s] equal to result's dist_val: %d. alt_wu_name set to: %s\n",
               //rp->output_files[i].file_info->name, rp->dist_val, wup->alt_name.c_str());
        //       rp->output_files[i].file_info->name, rp->dist_val, rp->output_files[i].file_info->alt_wu_name.c_str());
        //fflush(stdout);

        rp->output_files[i].file_info->dist_val = rp->dist_val;
...
bool CLIENT_STATE::garbage_collect_always() {
    ...
    /// BOINC-MR DEBUG
    //msg_printf(0, MSG_INFO, "Deleting pers_file_xfers without referenced files. Num: %d", pers_file_xfers->pers_file_xfers.size());

    /// BOINC-MR TODO : instead of relying on <sticky/> tag, count number of file_info references here
    /// depends on number of active connections (files being server AND downloading from mapper)
    ...
    while (pfx_iter != pers_file_xfers->pers_file_xfers.end()) {
        ...
        if (pfx->fip->ref_cnt == 0) {
            ...
            /// BOINC-MR DEBUG
            msg_printf(0, MSG_INFO, "Deleting pers_file_xfer without referenced files. File: %s", pfx->fip->name);
    ...
    while (fi_iter != file_infos.end()) {
        ...
        if (fip->ref_cnt==0) {
            ...
            if (fip->mr_file_type == MR_REDUCE_INPUT){
                printf("Deleting unreferenced file %s. Sticky: %dn", fip->name, fip->sticky);
                fflush(stdout);
            }
...
int CLIENT_STATE::quit_activities() {
    // BOINC-MR DEBUG
    printf("Inside CLIENT_STATE::quit_activities. Going to call mr_suspend.\n");
    fflush(stdout);
    ...
    /// BOINC-MR TODO - set boincerror value
    // terminate existing client connections and stop listening
    retval = mr_client_connections->mr_terminate_connections();
    if (retval) {
        msg_printf(NULL, MSG_INTERNAL_ERROR,
            "Couldn't terminate MapReduce client connections: %s", boincerror(retval)
        );
    }

    mr_file_server.mr_suspend();
