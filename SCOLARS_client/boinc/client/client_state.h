...
public:
    ...
    // BOINC-MR
    int mr_get_output_port();
        /// returns currently defined port for uploading (map) output in mapreduce jobs
    void mr_set_output_port(int);
...
public:
    ...
    // keeps list of active client connections, files being served and users with access to them
    MR_CLIENT_CONNECTION_SET* mr_client_connections;
    MR_CLIENT_XFER_SET* mr_client_xfers;
        // set of connections to other clients to download input files
    int mr_client_xfer_giveup_period;
        // give up on trying to download from Mapper(s) after N seconds
    int mr_max_client_conn_attempts;
        // number of max connection attempts to Mappers by each persistent file transfer before giving up

private:

    bool mr_handle_file_uploads();
    // scan all FILE_INFOs and MR_CLIENT_CONNECTIONs.Start and stop serving files as needed/required
...
// BOINC-MR
#define MR_CLIENT_CONNECTION_START_PERIOD  1.0
    // how often to see if new files should start being served
#define MR_CLIENT_XFER_POLL_PERIOD   1.0
    // how often to poll file downloads from (Reduce) clients
