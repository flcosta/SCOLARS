...
int CLIENT_STATE::make_scheduler_request(PROJECT* p) {
    ...
    // BOINC-MR DEBUG
    //printf("BOINC-MR Debug: Entered CLIENT_STATE::make_scheduler_request\n");
    ...
    for (i=0; i<file_infos.size(); i++) {
        ...
        // report mapreduce info if this file belongs to a mapreduce job - what info to send?
        // <user_access> list; mr_status; mr_job_id; ?mr_file_type?; mr_output_port
        /// BOINC-MR TODO - edit info below
        if (fip->mapreduce) {
            char file_type_temp[256], mr_status_temp[256];

            if (fip->mr_file_type == MR_MAP_OUTPUT) strcpy(file_type_temp, "map_output");
            else if (fip->mr_file_type == MR_REDUCE_INPUT) strcpy(file_type_temp, "reduce_input");

            if (fip->mr_status == MR_OUTPUT_FILE_UPLOADING) strcpy(mr_status_temp, "mr_file_uploading");
            else if (fip->mr_status == MR_OUTPUT_FILE_DONE) strcpy(mr_status_temp, "mr_file_done");
            else if (fip->mr_status == MR_INPUT_FILE_AVAILABLE) strcpy(mr_status_temp,"mr_input_available");
            else if (fip->mr_status == MR_INPUT_FILE_WAITING) strcpy(mr_status_temp, "mr_input_waiting");
            ...
            "            <%s/>\n"
            "            <%s/>\n",
            fip->name, fip->nbytes, fip->status, fip->mr_job_id, file_type_temp, mr_status_temp
            );

            /// BOINC-MR TODO - report file by file output port OR general MR output port for all files
            // only prinf output port info if this is a map output
            if (fip->mr_status == MR_OUTPUT_FILE_UPLOADING)
                fprintf(f, "            <output_port>%d</output_port>\n", gstate.mr_get_output_port());

            /// BOINC-MR TODO - problem here? too many individual writes?
            // solution: strcat (dest_string, src_string)
            // char dest_string[k] ; k = mr_file_users_list.size() * X ; X = size of each user IP (50?) - Check if valid [Set by server]
            // strncat(dest, src, length); length = number of bytes to copy (must make sure "src" fits into "dest" first compute dest.len() - src.len() otherwise string does not end with /0!)
            for (i=0; i<fip->mr_file_users_list.size(); i++) {
                xml_escape(fip->mr_file_users_list[i].c_str(), buf, sizeof(buf));
                fprintf(f, "            <user_access>%s</user_access>\n", buf);
            }
            ...
            // BOINC-MR DEBUG
            printf("BOINC-MR Debug: Inside CLIENT_STATE::make_scheduler_request - after MapReduce info added to request\n");

        }
        // "Normal" (non-MapReduce) application
        else {
            fprintf(f,
                "    <file_info>\n"
                "        <name>%s</name>\n"
    ...
    /// BOINC-MR TODO: what if client is unable to host map outputs? How to check (e.g. previous execution, small file upload); how to change wu template
    /// to return the actual output, instead of just storing it locally (server must be ready for this situation)
    // BOINC-MR: let server know that this is a client capable of running a MapReduce job (host and upload map outputs; no need to return them to server)
    fprintf(f, "<mr_client/>\n");
...
int CLIENT_STATE::handle_scheduler_reply(PROJECT* project, char* scheduler_url) {
    ...
    // BOINC-MR DEBUG
    printf("[***] Entered CLIENT_STATE::handle_scheduler_reply\n");
    ...
    // BOINC-MR DEBUG
    //printf("[***] Inside CLIENT_STATE::handle_scheduler_reply - checking for apps\n");
    ...
    // BOINC-MR DEBUG
    //printf("[***] Inside CLIENT_STATE::handle_scheduler_reply - checking for file_infos");
    FILE_INFO* fip;
    ...
    // BOINC-MR DEBUG
    //printf("[***] Inside CLIENT_STATE::handle_scheduler_reply - checking for file_deletes");
    ...
    // BOINC-MR DEBUG
    //printf("[***] Inside CLIENT_STATE::handle_scheduler_reply - checking for app_versions\n");
    ...
    // BOINC-MR DEBUG
    //printf("[***] Inside CLIENT_STATE::handle_scheduler_reply - checking for workunits\n");
    //printf("Number of workunits in SCHED_REPLY [sr]: %d\n", sr.workunits.size());
    //fflush(stdout);
    unsigned int k;
    for (i=0; i<sr.workunits.size(); i++) {

        // BOINC-MR DEBUG
        //printf("SCHED_REPLY [sr].workunit[%d]: %s\n", i, sr.workunits[i].name);
        //fflush(stdout);

        if (lookup_workunit(project, sr.workunits[i].name)) continue;

        // D_VAL DEBUG
        printf("CLIENT_STATE::handle_scheduler_reply. This \"new\" workunit[%d] %s is not linked to project. Calling link_workunit()\n", i, sr.workunits[i].name);
        fflush(stdout);
        ...
        *wup = sr.workunits[i];
        ...
        // BOINC-MR DEBUG
        //printf("CS::handle_scheduler_reply - Added workunit: %s\n", wup->name);
        //fflush(stdout);

        /// BOINC-MR DEPRECATED
        // give all its file infos the necessary info on map addresses
        /*FILE_INFO* f;
        //file_ref {FILE_INFO* file_info;}
        for (k=0; k<wup->input_files.size(); k++){
            f = wup->input_files[k].file_info;
            if (f->mapreduce && f->mr_job_id > 0)
                f->mr_add_mapper_addresses(wup->mr_temp_mappers_ip_addrs);
            //struct file_info
            //out.printf("        <mr_mapper>%s</mr_mapper>\n", mr_mappers_ip_addrs[i].c_str());
        }*/
    ...
    // BOINC-MR DEBUG
    //printf("[***] Inside CLIENT_STATE::handle_scheduler_reply - checking for results\n");
    ...
    // BOINC-MR DEBUG
    //printf("[***] Inside CLIENT_STATE::handle_scheduler_reply - checking for acked results\n");
    ...
    // BOINC-MR DEBUG
    //printf("[***] Inside CLIENT_STATE::handle_scheduler_reply - checking for abort requests\n");
    ...
    for (i=0; i<sr.result_abort.size(); i++) {
        ...
        if (rp) {
            /// BOINC-MR DEBUG
            msg_printf(NULL, MSG_INTERNAL_ERROR, "Aborting task [cancelled]: %s", sr.result_abort[i].name);
    ...
    for (i=0; i<sr.result_abort_if_not_started.size(); i++) {
        ...
        if (rp->not_started()) {
            /// BOINC-MR DEBUG
            msg_printf(NULL, MSG_INTERNAL_ERROR, "Aborting task [before it started]: %s", sr.result_abort_if_not_started[i].name);
            rp->abort_inactive(ERR_ABORTED_BY_PROJECT);
        }
    }

    // D_VAL
    // check for file requests (for outputs of D_VAL workunits - validated by clients)
    int j;
    FILE_INFO* temp_fip;
    std::size_t found;
    std::string temp_fname, cur_wu_name, cur_wu_name_index;
    int client_index;
    char path[1024];
    double fsize;
    if (sr.dval_req_inputs.size() > 0){
        for (i=0; i<sr.dval_req_inputs.size(); i++){
            //cur_wu_name_index = sr.dval_req_inputs[i];
            cur_wu_name = sr.dval_req_inputs[i];

//            std::istringstream iss(cur_wu_name_index);
//
//            // the string inside the <return_input> tag should follow this organization
//            if (!(iss >> cur_wu_name >> client_index)){
//                msg_printf(NULL, MSG_INTERNAL_ERROR, "Wrong template used by server. Text found inside <return_input>: %s", cur_wu_name_index);
//                continue;
//            }

            // go through all input (generated_locally) file infos and try to find this wu name in its name
            for (j=0; j<file_infos.size(); j++){
                temp_fip = file_infos[j];
                // check if it is input
                if (temp_fip->generated_locally)
                    continue;

                // search filename for substring
                temp_fname = std::string(temp_fip->name);
                found = temp_fname.find(cur_wu_name);
                if (found!=std::string::npos){

                    // D_VAL DEBUG
                    msg_printf(NULL, MSG_INFO, "Found an input file [%s] which belongs to a required WU name: %s", temp_fip->name, cur_wu_name.c_str());

                    get_pathname(temp_fip, path, sizeof(path));
                    retval = file_size(path, fsize);
                    if (!retval){
                        temp_fip->nbytes = fsize;

                        // D_VAL DEBUG
                        msg_printf(NULL, MSG_INFO, "Successfully set file size [nbytes:%f] for file: %s", temp_fip->nbytes, cur_wu_name.c_str());

                    }
                    else{
                        // D_VAL DEBUG
                        msg_printf(NULL, MSG_INTERNAL_ERROR, "handle_scheduler_reply: Error. Could not get file size for file[%s]: %d", temp_fip->name, retval);
                        continue;
                    }

                    temp_fip->requested_file = true;
                    temp_fip->urls.push_back(sr.dval_req_url);
                    continue;
                }
            }
        }
    }
    if (sr.dval_req_outputs.size() > 0){
        for (i=0; i<sr.dval_req_outputs.size(); i++){
            cur_wu_name = sr.dval_req_outputs[i];

            // go through all output file infos and try to find this wu name in its name
            for (j=0; j<file_infos.size(); j++){
                temp_fip = file_infos[j];
                ...
                // if the result is still defined, compare against the wu_name
                if (temp_fip->result){
                        if (temp_fip->result->wu_name){
                            // search wu name for substring
                            temp_fname = std::string(temp_fip->result->wu_name);
                            found = temp_fname.find(cur_wu_name);
                            if (found!=std::string::npos){

                            // D_VAL DEBUG
                            msg_printf(NULL, MSG_INFO, "Found an output file [%s] with wu_name [%s] which corresponds to a required WU name: %s",
                                       temp_fip->name, temp_fip->result->wu_name, cur_wu_name.c_str());
                ...
                if (found!=std::string::npos){
                    ...
                    // D_VAL DEBUG
                    msg_printf(NULL, MSG_INFO, "Found an output file [%s] which belongs to a required WU name: %s", temp_fip->name, cur_wu_name.c_str());
    ...
    // BOINC-MR DEBUG
    //printf("Leaving CLIENT_STATE::handle_scheduler_reply.");
...
const char *BOINC_RCSID_d35a4a7711 = "$Id: cs_scheduler.cpp 20785 2010-03-03 23:52:20Z davea $";
