...
// D_VAL
// value set by server in for distributed validation workunits - client files need to be validated, so there is no hash available yet
#define D_VAL_NO_CHECKSUM_VALUE "zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz"
...
bool CLIENT_STATE::start_new_file_xfer(PERS_FILE_XFER& pfx) {
    ...
    // BOINC-MR DEBUG
    /*msg_printf(NULL, MSG_INFO, "start_new_fil_xfer(): Number of file_xfers: %d || Number of client_xfers: %d\n"
                    "Max file_xfers [Project]: %d | Max file_xfers [Total]: %d || Max client_xfers [P]: %d | Max client_xfers [T]: %d",
               file_xfers->file_xfers.size(), mr_client_xfers->client_xfers.size(),
               config.max_file_xfers_per_project, config.max_file_xfers, config.max_client_xfers_per_project, config.max_client_xfers);*/

    if (network_suspended) return false;

    // BOINC-MR
    // normal transfer
    if (pfx.fip->mr_file_type != MR_REDUCE_INPUT){
        ...
        //msg_printf(NULL, MSG_INFO, "start_new_fil_xfer(): File Transfers. nproj: %d  || ntotal: %d", nproj, ntotal);
    ...
    // client transfer
        ...
        for (i=0; i<mr_client_xfers->client_xfers.size(); i++) {
            MR_CLIENT_XFER* cxp = mr_client_xfers->client_xfers[i];
            ntotal++;
            if (pfx.fip->project == cxp->mr_fip->project) {
                nproj++;
            }
        }

        //msg_printf(NULL, MSG_INFO, "start_new_fil_xfer(): Client Transfers. nproj: %d  || ntotal: %d", nproj, ntotal);

        if (nproj >= config.max_client_xfers_per_project) return false;
        if (ntotal >= config.max_client_xfers) return false;
    }
    return true;
}
...
int FILE_INFO::verify_file(bool strict, bool show_errors) {
    ...
    if (signature_required) {
        ...
        if (strcmp(cksum, md5_cksum)) {
            ...
            // D_VAL
            // check if the md5 corresponds to predefined value (which means this file came from another client, to be validated and therefore has no md5 yet)
            if (!strcmp(D_VAL_NO_CHECKSUM_VALUE, md5_cksum)){
                // D_VAL DEBUG
                msg_printf(project, MSG_INFO,
                    "File %s is part of distributed validation workunit (has a predefined checksum value: %s)", name, md5_cksum
                );
            }
            // normal workunit. If the checksum doesn't match with the value sent by server, set error
            else{
                if (show_errors) {
                    msg_printf(project, MSG_INTERNAL_ERROR,
...
bool CLIENT_STATE::handle_pers_file_xfers() {
    ...
    for (i=0; i<file_infos.size(); i++) {
        ...
        // D_VAL DEBUG
//        if (fip->status == FILE_PRESENT && fip->requested_file){
//            msg_printf(NULL, MSG_INFO,
//                "handle_pers_file_xfer: File: %s. fip->requested_file: %d | fip->uploaded: %d",
//                fip->name, fip->requested_file, fip->uploaded
//            );
//        }
        ...
        /// D_VAL TODO: check if this is working
        // If this file was requested by server, start uploading it if available
        else if(fip->requested_file && fip->status == FILE_PRESENT && !fip->uploaded){

            // D_VAL DEBUG
            msg_printf(fip->project, MSG_INFO,
                "handle_pers_file_xfer: Creating a new PERS_FILE_XFER for upload of requested file: %s", fip->name
            );
    ...
    while (iter != pers_file_xfers->pers_file_xfers.end()) {
        ...
        if (pfx->pers_xfer_done) {
            ...
            // D_VAL
            // set uploaded = true, even this is an input file which was requested by server
            } else if (!fip->uploaded && fip->requested_file){
                fip->uploaded = true;
...
const char *BOINC_RCSID_66410b3cab = "$Id: cs_files.cpp 19103 2009-09-18 20:49:54Z davea $";
