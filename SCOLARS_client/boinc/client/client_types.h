...
// BOINC-MR
// Values to identify file as map app output or reduce app input
// BOINC-MR TODO - two other values needed for Map input and reduce output?
//
#define MR_MAP_OUTPUT       0
#define MR_REDUCE_INPUT     1

// MapReduce status - two alternatives for each file status. Default=-1 (not set yet)
// If FILE_PRESENT [Output]:
// - MR_OUTPUT_FILE_UPLOADING - (map) output file should be uploading;
// - MR_OUTPUT_FILE_DONE - (map) output file already uploaded or not necessary to upload;
// If FILE_NOT_PRESENT [Input]:
// - MR_INPUT_FILE_WAITING - (reduce) input file is not yet available from mapper;
// - MR_INPUT_FILE_AVAILABLE - (reduce) input file is available for download from mapper;
//
#define MR_OUTPUT_FILE_UPLOADING   0
#define MR_OUTPUT_FILE_DONE        1
#define MR_INPUT_FILE_WAITING      2
#define MR_INPUT_FILE_AVAILABLE    3

#define MR_DEFAULT_PORT            8017

/// BOINC-MR TODO: follow FILE_XFER_BACKOFF, but what changes? How to control backoff since users may be unavailable randomly?
/// Variable to change in tests?
/// Save in FILE_INFO instead of Project, since each file has differente download addresses, unlike BOINC's unique download location per project
// client xfer backoff state for a project
/// DIST_VERSION
// if client_xfer_failures exceeds CLIENT_XFER_FAILURE_LIMIT,
// we warn Scheduler to scheduler more Map tasks
// NOTE: this refers to transient failures, not permanent.
//
#define MR_CLIENT_XFER_FAILURE_LIMIT 150
struct MR_CLIENT_XFER_BACKOFF {
    int client_xfer_failures;
        // count of consecutive failures
    double next_xfer_time;
        // when to start trying again
    bool ok_to_transfer();
    void client_xfer_failed(char*);
    void client_xfer_succeeded();

    MR_CLIENT_XFER_BACKOFF() {
...
struct FILE_INFO {
    ...
    // BOINC-MR
    //
    //std::vector<std::string> mappers_addrs;
    int start_mapper_addr;
    int current_mapper_addr;
        // Used by client when downloading Reduce input from other clients that
        // have run Map tasks and have their output

    bool mapreduce;
        // Whether it is part of a MapReduce job
    int mr_job_id;
        // MapReduce job id it belongs to
    int mr_file_type;
        // File type - Map output or Reduce input
    int mr_status;
        // MapReduce status
    char mr_wu_name[256];
        // Map Workunit name - required to check if this file is to be uploaded to Reducer [mr_check_req_file]
    std::vector<std::string> mr_file_users_list;
        // list of users (IPs) allowed to download this file [TAG: <user_access> x.x.x.x </user_access>
    double mr_serving_start_time;
        // instant when first started serving the file
    double mr_serving_time_so_far;
        // Total time this file has been available for upload (not necessarily uploading)
    double mr_serving_last_time;
        /// when the above was last updated.
        /// Defined only while file is being served / available for upload

    bool mr_permanent_cxf_error;                 /// BOINC-MR TODO: unnecessary if file is not available in central server! CENTRALIZED_VERSION
        // true if there was a permanent error during download from client (mapper)
        // e.g.: timed out trying to connect to Mappers

    bool mr_hash;
        // whether this is a hash file (checksum of an output file), to be returned to server

    // Info for Reduce tasks - downloading data from Mappers
    std::vector<std::string> mr_mappers_ip_addrs;
        // IP addresses of users who have Map outputs (needed for Reduce tasks)
    std::vector<std::string> mr_get_mappers_addrs(){
        return mr_mappers_ip_addrs;
    };

    // D_VAL
    //std::string alternative_name;
        /// DEPRECATED - use alt_wu_name instead
        // alternative name used for obfuscation, when using distributed validation
    std::string alt_wu_name;
        // alternative name for this file's WU, used for obfuscation purposes (distributed validation)
    bool dist_val;
        // true if this (output) file is part of Result to be validated by another client (instead of server). Therefore, this output file will be available for upload
    bool c2c_transfer;
        // true if this (input) file is a candidate for client-to-client transfer (download it from other client instead of downloading from server)

    bool requested_file;
        // true if this file was requested by server (to be uploaded). It was either an output of a D_VAL WU, or the input for a validator WU (task that was validating
        // another client WU).

    // C2C
    bool c2c_upload;
        // true if this input file must be available for upload [tag <c2c_up/> found in FILE_INFO] (does not have to be map output or d_val input)

    /// BOINC-MR DEPRECATED
    //int mr_add_mapper_addresses(std::vector<std::string>);

    const char* get_init_mapper_addr();
    const char* get_next_mapper_addr();
    const char* get_current_mapper_addr();

    MR_CLIENT_XFER_BACKOFF mr_client_download_backoff;
    inline MR_CLIENT_XFER_BACKOFF& mr_client_xfer_backoff() {
        return mr_client_download_backoff;
    }

    // port where to host (map) output file - client-defined
    /// BOINC-MR TODO necessary here? Or only in general preferences (client_state.xml) - 1st Version: only in general preferences (listen only in 1 port)
    //int mr_output_port;
...
// NOTE: this refers to transient failures, not permanent.
//
#define FILE_XFER_FAILURE_LIMIT 300
...
// BOINC-MR
// Values to identify application as Map or Reduce
#define MR_MAP_APPLICATION   0
#define MR_REDUCE_APPLICATION   1
struct APP {
    char name[256];
    ...
    // MapReduce job id
    int mr_job_id;
    // Type of MR application (Map or Reduce)
    int mr_app_type;
...
struct WORKUNIT {
    ...
    /// BOINC-MR DEPRECATED
    /// needed for now to get mappers' address from the server
    //std::vector<std::string> mr_temp_mappers_ip_addrs;

    // D_VAL
    //char alt_name[100];
    std::string alt_name;
        // alternative name given to this workunit. Used for obfuscation purposes.

    /*std::vector<std::string> mr_get_temp_mappers_addrs(){
        return mr_temp_mappers_ip_addrs;
    }*/
};
...
struct RESULT {
    ...
    /// D_VAL
    bool dist_val;
        // true if tag <dist_val/> was found. This means that this result is to be validated by another client, instead of server.
        // All its output files will be made available for upload.
};
