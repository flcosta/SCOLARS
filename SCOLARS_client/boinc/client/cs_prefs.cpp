...
// BOINC-MR
// Port currently defined for receiving download requests from other clients (Map output = Reduce task input)
//
int CLIENT_STATE::mr_get_output_port(){

    // BOINC-MR DEBUG
    //printf("BOINC-MR Debug: Inside CLIENT_STATE::mr_get_output_port\n");

    return global_prefs.mr_output_port;
}

// BOINC-MR
// Set port for inter-client transfers
//
void CLIENT_STATE::mr_set_output_port(int port){

    // BOINC-MR DEBUG
    //printf("BOINC-MR Debug: Inside CLIENT_STATE::mr_set_output_port\n");

    global_prefs.mr_set_output_port(port);
}
