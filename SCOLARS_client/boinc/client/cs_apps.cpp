...
bool CLIENT_STATE::handle_finished_apps() {
    ...
    while (iter != active_tasks.active_tasks.end()) {
        atp = *iter;
        // BOINC-MR DEBUG
        //printf("Active task %s [id:%d] state: %d\n", atp->result->name, atp->pid, atp->task_state());
        //fflush(stdout);
...
ACTIVE_TASK* ACTIVE_TASK_SET::lookup_pid(int pid) {
    ...
    // BOINC-MR DEBUG
    //msg_printf(NULL, MSG_INFO, "Entered lookup_pid()");

    for (i=0; i<active_tasks.size(); i++) {
        atp = active_tasks[i];

        // BOINC-MR DEBUG
        //msg_printf(NULL, MSG_INFO, "Comparing given pid [%d] with task %s pid: %d", pid, atp->result->name, atp->pid);
...
const char *BOINC_RCSID_7bf63ad771 = "$Id: cs_apps.cpp 20117 2010-01-09 16:41:17Z davea $";
