...
int FILE_XFER::init_upload(FILE_INFO& file_info) {
    ...
    // D_VAL DEBUG
    msg_printf(file_info.project, MSG_INFO,
        "[fxd] init_upload. upload_offset %.0f", file_info.upload_offset
    );
    ...
    if (file_info.upload_offset < 0) {
        ...
        /// BOINC-MR DEBUG
        fprintf(stderr, "signed_xml (what to sign):\n%s\nxml_sig:%s\n",
                file_info.signed_xml,
                file_info.xml_signature);
        fflush(stderr);
...
const char *BOINC_RCSID_31ba21bea3 = "$Id: file_xfer.cpp 20750 2010-02-27 01:04:14Z davea $";
