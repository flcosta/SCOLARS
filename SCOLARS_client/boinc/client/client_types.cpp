...
int PROJECT::write_state(MIOFILE& out, bool gui_rpc) {
    ...
    // BOINC-MR DEBUG
    //printf("Entered PROJECT::write_state");
    ...
    // BOINC-MR DEBUG
    //printf("Left PROJECT::write_state\n");
...
int APP::parse(MIOFILE& in) {
    ...
    // BOINC-MR DEBUG
    //printf("Mar 2010 [***] BOINC-MR Debug: Entered APP::parse");
    ...
    while (in.fgets(buf, 256)) {
        ...
        // BOINC-MR DEBUG
        //printf("BOINC-MR Debug: Calling APP::parse_mapreduce\n");

        // call App_Parse_MapReduce in mr_tasktracker.cpp OR parse here - create MR object or just get items directly (job id, task type)
        if (match_tag(buf, "<mapreduce>")) {
            mapreduce = true;
            // get MapReduce variables
            retval = parse_mapreduce(in);
...
int APP::write(MIOFILE& out) {
    ...
    // BOINC-MR DEBUG
    //printf("BOINC-MR Debug: Entered APP::write - writing different string if 'mapreduce==true'\n");

    // default if not mapreduce app
    if (!mapreduce){
        out.printf(
            "<app>\n"
            "    <name>%s</name>\n"
    ...
    // if this is a part of a MapReduce job
    else {
        // BOINC-MR DEBUG
        printf("BOINC-MR Debug: Inside APP::write - mapreduce==true | Writing MR info\n");
        char type[128];
        if (mr_app_type == MR_MAP_APPLICATION)
            strcpy(type, "<map/>");
        else
            strcpy(type, "<reduce/>");
        out.printf(
            ...
            "    </mapreduce>\n"
            "</app>\n",
            name, user_friendly_name, type, mr_job_id
        );
    }
...
// BOINC-MR
// parse MapReduce info from inside <mapreduce> tag in workunit template
int APP::parse_mapreduce(MIOFILE& in){

    // BOINC-MR DEBUG
    printf("BOINC-MR Debug: Entered APP::parse_mapreduce\n");
    ...
    while (in.fgets(buf, 256)) {
        ...
        // BOINC-MR: If no job id has been set, handle task as non-mapreduce
        if (match_tag(buf, "</mapreduce>")) {
            if (mr_job_id == 0) mapreduce = false;
            return 0;
        }

        // MapReduce job id
        if (parse_int(buf, "<job_id>", mr_job_id)) continue;

        // Map or Reduce Application
        if (match_tag(buf, "<map/>")){
            mr_app_type = MR_MAP_APPLICATION;
            continue;
        }
        if (match_tag(buf, "<reduce/>")) {
            mr_app_type = MR_REDUCE_APPLICATION;
            continue;
        }

        if (log_flags.unparsed_xml) {
            msg_printf(0, MSG_INFO,
                "[unparsed_xml] APP::parse_mapreduce(): unrecognized: %s\n", buf
            );
        }
...
FILE_INFO::FILE_INFO() {
    ...
    mapreduce = false;
    strcpy(mr_wu_name, "");
    mr_file_users_list.clear();
    mr_serving_time_so_far = 0;
    mr_mappers_ip_addrs.clear();
    mr_file_type = -1;
    start_mapper_addr = -1;
    current_mapper_addr = -1;
    mr_hash = false;                // whether it's a hash file
    /// BOINC-MR TODO: unnecessary? If permanent error, pfx calls permanent_failure()
    mr_permanent_cxf_error = false;
    /// BOINC-MR TODO
    // info on general preferences, not file-specific
    // If changed while uploading, do not terminate connection, but inform server of new port (new connections will go through new port)
    //mr_file_output_port = gstate.get_mr_output_port();
    //mr_output_port = MR_DEFAULT_PORT;
    mr_status = -1;
...
int FILE_INFO::parse(MIOFILE& in, bool from_server) {
    ...
    char mr_mapper_addr[256];
    char mr_mapper_addresses[1024];          // parse <mr_mappers>
    char* token;                            // token of <mr_mappers> IP addresses
    std::string url;
    PERS_FILE_XFER *pfxp;
    int retval;

    // BOINC-MR DEBUG
    //msg_printf(0, MSG_INFO, "[BOINC-MR DEBUG] FILE_INFO::parse() Parsing: %s\n", buf);

    while (in.fgets(buf, 256)) {

        // BOINC-MR DEBUG
        printf("[BOINC-MR DEBUG] FILE_INFO::parse() Parsing: %s", buf);
        fflush(stdout);
        ...
        // BOINC-MR
        // check if this file is the checksum/hash of an output file, to be returned to lower communication costs
        if (parse_bool(buf, "mr_hash", mr_hash)){
            // BOINC-MR
            printf("[BOINC-MR DEBUG] File_info::parse() Found <mr_hash/>\n");
            continue;
        }

        // BOINC-MR
        // all host addresses within same tag, separated by blank space (e.g.: "192.1.10.2 226.11.2.1 403.4.1.12")
        if (parse_str(buf, "<mr_mappers>", mr_mapper_addresses, sizeof(mr_mapper_addresses))) {
            // BOINC-MR DEBUG
            printf("[BOINC-MR DEBUG] File_info::parse() Found <mr_mappers>:\n %s\n", mr_mapper_addresses);
            fflush(stdout);
            // go through addresses, and add them to list
            token = strtok(mr_mapper_addresses, " ");
            while (token != NULL) {

                // BOINC-MR DEBUG
                printf("[BOINC-MR DEBUG] File_info::parse() Got Mapper Address: %s\n", token);
                fflush(stdout);

                // extract the host from the no_proxy url
                mr_mappers_ip_addrs.push_back(token);
                token = strtok(NULL, " ");
            }
            continue;
        }
        ...
        // D_VAL
        // all host addresses within same tag, separated by blank space (e.g.: "192.1.10.2 226.11.2.1 403.4.1.12")
        /// TODO - use the same tag for both mappers and clients with outputs to be dist validated (e.g.: <c2c>10.10.1.12</c2c> - client-to-client)
        if (parse_str(buf, "<c2c>", mr_mapper_addresses, sizeof(mr_mapper_addresses))) {
            // D_VAL DEBUG
            printf("[D_VAL DEBUG] File_info::parse() Found <c2c>:\n %s\n", mr_mapper_addresses);
            fflush(stdout);

            // save this file_info as a client-to-client transfer candidate
            c2c_transfer = true;
            ...
            while (token != NULL) {
                ...
                // D_VAL DEBUG
                printf("[D_VAL DEBUG] File_info::parse() Got Client Address: %s\n", token);
                fflush(stdout);
        ...
        if (parse_bool(buf, "dist_val", dist_val)) continue;

        if (parse_bool(buf, "c2c_up", c2c_upload)) continue;


        /// BOINC-MR TODO: DEPRECATED
        /*if (parse_str(buf, "<mr_mapper>", mr_mapper_addr, sizeof(mr_mapper_addr))) {

            // BOINC-MR DEBUG
            msg_printf(0, MSG_INFO, "[BOINC-MR DEBUG] File_info::parse() Got Mapper Address: %s\n", mr_mapper_addr);

            mr_mappers_ip_addrs.push_back(mr_mapper_addr);
            continue;
        }*/


        // call FILE_INFO::parse_mapreduce
        if (match_tag(buf, "<mapreduce>")) {
            mapreduce = true;
            // get MapReduce variables
            retval = parse_mapreduce(in);
        ...
        /// D_VAL DEPRECATED - saved in WORKUNIT
        // Dist-validation: this alternative name is used for obsfucation purposes
        // When hosting this file, accept requests for this name instead of the original
        //if (parse_str(buf, "<alt_name>", alternative_name)) continue;
...
// BOINC-MR
// parse info inside mapreduce tags in file_info
//
int FILE_INFO::parse_mapreduce(MIOFILE& in){

    // BOINC-MR DEBUG
    //printf("BOINC-MR Debug: Entered FILE_INFO::parse_mapreduce\n");

    /// BOINC-MR TODO - deprecated? inside <mapreduce> tags?
    char mr_mapper_addresses[1024];          // parse <mr_mappers>
    char* token;                            // token of <mr_mappers> IP addresses
    ...
    while (in.fgets(buf, 256)) {
        ...
        // BOINC-MR DEBUG
        printf("[BOINC-MR DEBUG] FILE_INFO::parse_mapreduce() Parsing: %s", buf);
        fflush(stdout);

        // to send file info to server with all MapReduce info
        safe_strcat(signed_xml, buf);

        // B-MR: If no job id has been set, handle task as non-mapreduce
        if (match_tag(buf, "</mapreduce>")) {
            // BOINC-MR
            printf("BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found </mapreduce> tag\n");
            if (mr_job_id == 0){
                msg_printf(0, MSG_INFO, "FILE_INFO::parse_mapreduce(): No MapReduce Job Id found");
                mapreduce = false;
            }
            return 0;
        }

        /// BOINC-MR TODO : this is being repeated inside FILE_INFO::parse()
        ///     - decide if this goes inside <mapreduce> tags [when inserted by server]
        // all host addresses within same tag, separated by blank space (e.g.: "192.1.10.2 226.11.2.1 403.4.1.12")
        if (parse_str(buf, "<mr_mappers>", mr_mapper_addresses, sizeof(mr_mapper_addresses))) {
            // BOINC-MR DEBUG
            printf("[BOINC-MR DEBUG] File_info::parse() Found <mr_mappers>:\n %s\n", mr_mapper_addresses);
            ...
            while (token != NULL) {
                ...
                // BOINC-MR DEBUG
                printf(0, MSG_INFO, "[BOINC-MR DEBUG] File_info::parse() Found <mr_mappers>\n. Got Mapper Address: %s\n", token);
                fflush(stdout);
        ...
        /// BOINC-MR DEBUG
        if (match_tag(buf, "<job_id>")){

            //printf("BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found <job_id> tag");
            //fflush(stdout);

            const char* p = strstr(buf, "<job_id>");
            if (!p){
                fprintf(stderr, "BOINC-MR DEBUG == Did not find <job_id> [strstr]\n");
            }

            //const char* tag = "<job_id>";
            const char* q= p+strlen("<job_id>");
            //printf("BOINC-MR DEBUG == String to convert after <job_id> tag: %s", q);
            //fflush(stdout);

            int y = strtol(p+strlen("<job_id>"), 0, 0);
            mr_job_id = y;
            //printf(0, MSG_INFO, "BOINC-MR DEBUG Returned number [strtol]: %d\n", y);
            //fflush(stdout);
            continue;
        ...
        //if (parse_int(buf, "<job_id>", mr_job_id)) continue;
        /*if (parse_int(buf, "<job_id>", mr_job_id)){
            // BOINC-MR
            msg_printf(0, MSG_INFO, "BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found <job_id> tag; Job ID: %d\n", mr_job_id);
            continue;
        }*/
        ...
        /// MapReduce output port for Map results
        ///if (parse_int(buf, "<output_port>", mr_output_port)) continue;

        // Map output or Reduce input
        if (match_tag(buf, "<map_output/>")){
            // BOINC-MR
            printf("BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found <map_output/> tag\n");
            mr_file_type = MR_MAP_OUTPUT;
            continue;
        }
        if (match_tag(buf, "<reduce_input/>")) {
            // BOINC-MR
            printf("BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found <reduce_input/> tag\n");
            mr_file_type = MR_REDUCE_INPUT;
            continue;
        }

        /// TODO - confirm these tags (file_uploading; input [un]available)
        // If map output is supposed to be uploading (pers_file_xfer should be set as well)
        if (match_tag(buf, "<mr_file_uploading/>")) {
            // BOINC-MR
            printf("BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found <mr_file_uploading/> tag\n");
            // leave status as init (-1); mr_handle_file_uploads will restart upload
            // mr_status = MR_OUTPUT_FILE_UPLOADING
            continue;
        }

        // MapReduce job id
        if (parse_double(buf, "<serving_time>", mr_serving_time_so_far)){
            // BOINC-MR
            printf("BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found <serving_time> tag; Serving time so far: %f\n", mr_serving_time_so_far);
            fflush(stdout);
            continue;
        }


        /// TODO - necessary tag? Just leave it at =-1?
        // If map output does not need to be uploaded
        if (match_tag(buf, "<mr_file_done/>")) {
            // BOINC-MR
            printf("BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found <mr_file_done/> tag\n");
            mr_status = MR_OUTPUT_FILE_DONE;
            continue;
        }

        // If reduce input is not available yet (Mapper has not finished)
        if (match_tag(buf, "<mr_input_waiting/>")) {
            // BOINC-MR
            printf("BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found <mr_input_waiting/> tag\n");
            mr_status = MR_INPUT_FILE_WAITING;
            continue;
        }
        // If reduce input can be downloaded from other client (mapper)
        if (match_tag(buf, "<mr_input_available/>")) {
            // BOINC-MR
            printf("BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found <mr_input_available/> tag\n");
            mr_status = MR_INPUT_FILE_AVAILABLE;
            continue;
        }

        if (parse_str(buf, "<mr_wu_name>", mr_wu_name, sizeof(mr_wu_name))){
            // BOINC-MR DEBUG
            printf("BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found <mr_wu_name> tag: %s\n", mr_wu_name);
            continue;
        }


        // users that may have access to this file
        if (parse_str(buf, "<user_access>", user_access)) {
            // BOINC-MR
            printf("BOINC-MR DEBUG == Inside FILE_INFO::parse_mapreduce() found <user_access> tag\n");
            mr_file_users_list.push_back(user_access);
            continue;
        }

        if (log_flags.unparsed_xml) {
            printf("Error in FILE_INFO::parse_mapreduce - unparsed xml\n");
            //printf("[unparsed_xml] FILE_INFO::parse_mapreduce(): unrecognized: %s\n", buf);
            msg_printf(0, MSG_INFO,
                "[unparsed_xml] FILE_INFO::parse_mapreduce(): unrecognized: %s\n", buf);
        }
    }
...
int FILE_INFO::write(MIOFILE& out, bool to_server) {
    ...
    // BOINC-MR DEBUG
    //printf("BOINC-MR Debug: Entered FILE_INFO::write\n");
    //fflush(stdout);
    ...
    if (!to_server) {
        ...
        // D_VAL
        if (dist_val) out.printf("    <dist_val/>\n");
    }


    // BOINC-MR
    // check if this a hash file (to be returned instead of actual output file) - let server know
    if (mr_hash)
        out.printf("    <mr_hash/>\n");

    // BOINC-MR DEBUG
    //printf("BOINC-MR Debug: Inside FILE_INFO::write() Checking IF mapreduce\n");
    //fflush(stdout);

    // BOINC-MR
    if (mapreduce) {
        // BOINC-MR DEBUG
        //printf("BOINC-MR Debug: Inside FILE_INFO::write - 'mapreduce == true' | Writing MR file info\n");
        //fflush(stdout);

        out.printf("    <mapreduce>\n");

        // Not sent to server, just keep information on mappers addresses locally
        /// BOINC-MR TODO: next version - send info on available and unavailable mappers to update info on server
        //app->mr_app_type==MR_REDUCE_APPLICATION
        if (!to_server && mr_file_type == MR_REDUCE_INPUT){

            //msg_printf(NULL, MSG_INFO, "Inside FILE_INFO::write() File is a Reduce input and is not being sent to server."
              //         " Number of Mappers IP Addresses: %d\n", mr_mappers_ip_addrs.size());

            if (mr_mappers_ip_addrs.size() > 0){
                out.printf("        <mr_mappers>");
                for (i=0; i<mr_mappers_ip_addrs.size(); i++){
                    out.printf("%s", mr_mappers_ip_addrs[i].c_str());
                    if (i != (mr_mappers_ip_addrs.size()-1))
                        out.printf(" ");
                }
                out.printf("</mr_mappers>\n");
            }
        ...
        if (mr_job_id != 0) out.printf("        <job_id>%i</job_id>\n", mr_job_id);
        if (mr_file_type == MR_MAP_OUTPUT) out.printf("        <map_output/>\n");
        if (mr_file_type == MR_REDUCE_INPUT) out.printf("        <reduce_input/>\n");

        /// BOINC-MR TODO - saved here AND in general preferences - "global_prefs.xml/client_state.xml" (must tell scheduler where other clients should connect)
        if (to_server and mr_status == MR_OUTPUT_FILE_UPLOADING) out.printf("        <output_port>%i</output_port>\n", gstate.mr_get_output_port());

        if (mr_status == MR_OUTPUT_FILE_UPLOADING) out.printf("        <mr_file_uploading/>\n");
        if (mr_status == MR_OUTPUT_FILE_DONE) out.printf("        <mr_file_done/>\n");
        if (mr_status == MR_INPUT_FILE_AVAILABLE) out.printf("        <mr_input_available/>\n");
        if (mr_status == MR_INPUT_FILE_WAITING) out.printf("        <mr_input_waiting/>\n");

        if (!to_server) out.printf("        <serving_time>%f</serving_time>\n", mr_serving_time_so_far);
        if (!to_server && strlen(mr_wu_name)) out.printf("        <mr_wu_name>%s</mr_wu_name>\n", mr_wu_name);

        for (i=0; i<mr_file_users_list.size(); i++) {
            xml_escape(mr_file_users_list[i].c_str(), buf, sizeof(buf));
            out.printf("    <user_access>%s</user_access>\n", buf);
        }
        out.printf("    </mapreduce>\n");

        // BOINC-MR DEBUG
        //printf("BOINC-MR Debug: Leaving FILE_INFO::write - after writing MapReduce tag\n");
        //fflush(stdout);
    }

    // D_VAL
    // add information on client addresses, inside <c2c> tag
    if (c2c_transfer){
        if (mr_mappers_ip_addrs.size() > 0){
            out.printf("        <c2c>");
            for (i=0; i<mr_mappers_ip_addrs.size(); i++){
...
int WORKUNIT::parse(MIOFILE& in) {
    ...
    while (in.fgets(buf, sizeof(buf))) {
        ...
        // BOINC-MR DEBUG
        msg_printf(0, MSG_INFO, "[BOINC-MR DEBUG] WORKUNIT::parse() Parsing: %s\n", buf);
        ...
        /*if (parse_str(buf, "<mr_mapper>", mr_mapper_addr, sizeof(mr_mapper_addr))) {
            ...
            mr_temp_mappers_ip_addrs.push_back(mr_mapper_addr);
            continue;
        }*/

        //D_VAL
        if (parse_str(buf, "<alt_name>", alt_name)){
            continue;
            msg_printf(0, MSG_INFO, "[BOINC-MR DEBUG] File_info::parse() Found alt name: %s\n", alt_name.c_str());
        }
...
void RESULT::clear() {
    ...
    /// BOINC-MR TODO - report specified results immediately (instead of using cc_config.xml file to define it as a global preference)
...
int RESULT::parse_server(MIOFILE& in) {
    ...
    while (in.fgets(buf, 256)) {
        ...
        // D_VAL DEBUG
        printf("[D_VAL DEBUG] RESULT::parse_server() Parsing: %s", buf);
        fflush(stdout);
        ...
        // D_VAL
        if (parse_bool(buf, "dist_val", dist_val)){
            msg_printf(0, MSG_INFO,
                "[D_VAL] RESULT::parse_server(): Found dist_val\n", buf
            );
            continue;
        }
        if (log_flags.unparsed_xml) {
...
int RESULT::parse_state(MIOFILE& in) {
    ...
    while (in.fgets(buf, 256)) {
        ...
        // D_VAL DEBUG
        printf("[D_VAL DEBUG] RESULT::parse_state() Parsing: %s", buf);
        fflush(stdout);
        ...
        if (parse_bool(buf, "dist_val", dist_val)){
            // D_VAL DEBUG
            msg_printf(0, MSG_INFO,
                "[D_VAL] RESULT::parse_state(): Found dist_val\n", buf
            );
            continue;
        }
        if (log_flags.unparsed_xml) {
...
int RESULT::write(MIOFILE& out, bool to_server) {
    ...
    // BOINC-MR
    // Write MapReduce info for scheduler to know if this result if part of MapReduce job
    if (app->mapreduce && to_server){
        out.printf("    <mapreduce/>\n");
        out.printf("    <mr_job_id>%d</mr_job_id>\n",app->mr_job_id);
        if (app->mr_app_type == MR_MAP_APPLICATION)
            out.printf("    <map/>\n");
        else if (app->mr_app_type == MR_REDUCE_APPLICATION)
            out.printf("    <reduce/>\n");
    }
