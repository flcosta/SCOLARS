...
static void signal_handler(int signum) {
    ...
    // BOINC-MR DEBUG
    /// BOINC-MR TODO - handle signals received by mr client connections
	// make MR_FILE_SERVER process stop listening after receiving a signal
    gstate.mr_file_server.mr_suspend();

    msg_printf(NULL, MSG_INFO, "Inside signal_handler(). Received signal %d", signum);
    switch(signum) {
    case SIGHUP:
    case SIGINT:
    case SIGQUIT:
...
const char *BOINC_RCSID_f02264aefe = "$Id: main.cpp 20029 2009-12-23 19:52:34Z davea $";
